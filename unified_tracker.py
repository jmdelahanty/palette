...
def crop_frame_worker(frame_index, roi_sz, ds_thresh, se1, se4):
    """Worker to find and crop a single ROI."""
    try:
        ...
        return frame_index, roi, (x1, y1)
    except Exception:
        return None

def run_crop_stage(zarr_path):
    """Finds and saves ROIs for each frame."""
...
def track_roi_worker(frame_index, roi_thresh, se1, se2):
    """Worker to find keypoints within a pre-cropped ROI."""
    try:
        ...
        return frame_index, [heading] # Simplified return for now
    except Exception:
        return None

def run_tracking_stage(zarr_path):
    """Runs keypoint tracking on pre-cropped ROIs."""
    ...
    params = {'roi_thresh': 115, 'se1': disk(1), 'se2': disk(2)}
    serializable_params = {k: v for k, v in params.items() if not isinstance(v, np.ndarray)}
    serializable_params['morphology_disk_radii'] = {'se1': 1, 'se2': 2}
    track_group.attrs['tracking_parameters'] = serializable_params

    # NOTE: The tracked columns could be expanded to include all keypoints again
    tracking_results = track_group.create_dataset('tracking_results', shape=(num_images, 1), chunks=(4096, None), dtype='f8', overwrite=True)
    tracking_results[:] = np.nan
    tracking_results.attrs['column_names'] = ['heading_degrees']
