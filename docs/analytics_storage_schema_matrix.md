...
| `analysis/subject_shape_runs` | Body/eye/swim-bladder geometry, body frame, centerline, spline, and tail landmarks | historical v4/method 11; recording-bundle candidate v5/method 12 | Shared byte-planned access-aware candidate: row-aligned inner chunks near 128 KiB inside 8 MiB indexed shards; eager semantic axes near 1 MiB | exact recording subject-mask bundle rows for v5; explicit historical refined-mask rows for v4 | `fisheye.analysis.subject_shape_io` | shared atomic publisher; v5 remains selector-ineligible | no flattening requirement; specialized body geometry remains semantic |
