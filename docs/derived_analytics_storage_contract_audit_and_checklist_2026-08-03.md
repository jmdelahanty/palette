...
| Detection/session occupancy typed execution | coordination lane | implemented and activated in the current coordination checkpoint | Separate 30-array epoch and 29-array session logical equality; a closed `occupancy_v1` invocation; exact selected bbox/frame/score payload identities; resolved image dimensions and recording time axis; complete stimulus-window authority for epoch occupancy; exact scientific parameters; eleven measured phases; atomic selector-ineligible publication; direct/consolidated equivalence; and owned failure evidence | Both nominal fresh-process gates and detection-geometry, stimulus-window, and recording-axis post-request swap rejections pass; representative-scale physical-I/O tracing and real consumers remain pending |
