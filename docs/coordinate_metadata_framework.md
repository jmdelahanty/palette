...
- an optional controlled collection axis when one physical array stacks named
  scientific surfaces (for example subject-mask components); its exact ordered
  labels and cardinality come from a digest-bound authority and are distinct
  from the coordinate tuple's `components`;
...
`collection_axis` is part of the initial canonical-v2 grammar. Canonical-v2 has
not been released on `origin/main`, used by the current registry inventory, or
published in an authoritative cross-repository contract, so there is no earlier
deployed v2 grammar to preserve. The first Palette and external-consumer release
must include this field coherently. Do not publish or retain a partial v2 parser,
and do not add a permissive adapter that drops the axis. Any change after the
first canonical-v2 release requires ordinary schema-version evolution.

Run, subgroup, array name, shape, dimension name, value range, and historical
helper name are never coordinate authority.

Not every spatially derived value is itself a coordinate. Scalar distance,
speed, area, and angle arrays use an array-specific typed measurement/semantic
record when origin and X/Y axes are not applicable. That record still binds the
exact source coordinate descriptors, row and collection identity, operation,
units, and calibration or transform lineage. Do not invent a coordinate frame
for a scalar merely to reuse a descriptor, and do not treat a unit-bearing leaf
name as the measurement record.
...
These keys identify axis-0 rows; they do not name every axis in an array. A
`collection_axis`, such as ordered subject components within `(N, C, 2)`, says
which scientific member occupies each position inside one observation row. It
does not replace `instance_key` and does not create another observation. An
acquisition-frame array similarly maps each row to time but is not the row's
identity. Consumers validate all three independently when present: row identity,
temporal mapping, and collection membership.

The initial controlled collection roles are `subject_component`, `keypoint`,
`tail_segment`, and `chaser`. A role is semantic, not a synonym for an axis number. Its axis,
cardinality, ordered labels, and label-authority digest must all be explicit.
...
4. Stamp array-owned descriptors from sealed evidence, never reconstructed
   dictionaries.
5. Validate the staged binding while the run remains selector-ineligible.
6. Mark it complete without advancing normal selectors, reopen the authoritative
   path, and freshly validate the complete binding.
7. Advance parent selectors while the candidate is still ineligible, then flip
   selector eligibility last. A concurrent reader must therefore see either the
   prior valid run or the newly validated run, never a publishing candidate.
...
A selector-free diagnostic/training artifact may retain array-owned unbound
numeric semantics, but only by naming a registered immutable semantic profile.
That profile fixes the numeric space, geometry, components and units, axes,
pixel/edge convention, exact dtype, rank and trailing shape, axis-0 domain,
temporal/reference kind, source-digest kind, and derivation operation. Each
producer manifest also names one typed run-owned source-lineage record. Fresh
validation recomputes that record's digest and requires every array's reference
path and extent, frame count, mapping direction, and source/mapping digests to
resolve to it exactly. Dynamic evidence cannot be made authoritative merely by
repeating a value across array attrs. Such an artifact always declares
source-camera overlay and canonical promotion unsupported; an explicit binder
must create a new canonical run from exact authority rather than relabelling the
artifact in place.
...
The framework does not require numerical kernels to parse Zarr metadata or
carry descriptor dictionaries through every array operation. A producer or
reader boundary first loads sealed evidence, verifies that all inputs share a
compatible frame (or applies an exact persisted directed transform), and only
then passes ordinary arrays to smoothing, distance, speed, angle, interpolation,
or body-frame kernels. Those mathematical kernels should remain frame-agnostic.

Code must change when it currently selects dimensions, scales, crops, or
calibrations by convention; merely documenting that assumption is insufficient.
When the already persisted input values are proved to be in the intended frame,
the calculation and its numeric result need not change. A new output publisher
must still persist the exact input bindings, operation/selection lineage, output
row identity, and array-owned descriptor. If input values cannot be proved or
are shown to use the wrong frame, create a recomputed derived run rather than
relabeling the old values.

## Future derived-analysis layout

Future analysis writers should organize stable logical surfaces by entity,
quantity, processing level, and unit. For example,
`tracks/id_0/movement/speed/filtered/mm` is a sound logical path: it scopes one
track, then the movement quantity, then the filter level, then the numeric unit.
The hierarchy makes discovery and closed-inventory validation easier without
duplicating arrays.

The path is not scientific authority. Every numeric leaf still owns its exact
descriptor or typed measurement record, row-identity binding, input references,
and derivation lineage. Group attrs may declare a closed schema for their
children, but consumers must use a versioned logical reader and must not infer
units, coordinate space, axes, filter semantics, or overlay suitability from a
group or leaf name. A unit leaf such as `mm` means that the storage schema expects
millimetres; the bound record proves which calibration produced them.

Agent teams adding new derived surfaces should therefore:

1. Extend a versioned logical reader and the closed group/array inventory.
2. Reuse one authoritative numeric leaf per scientific surface; do not create
   aliases or per-member copies merely to make paths convenient.
3. Bind axis 0 to the correct identity domain and use a controlled collection
   axis for named members within a row.
4. Persist array-specific semantics and exact source/transform/calibration
   lineage before selector activation.
5. Test logical reader behavior, descriptor/measurement validation, stale or
   swapped lineage, and unsupported members or units—not only numeric values.

This layout policy applies to new future writers. Historical flat arrays remain
inspection or migration inputs and do not justify runtime adapters in normal
future-recording paths.
