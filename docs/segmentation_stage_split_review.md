...
| U-Net subject-mask inference | `src/fisheye/segmentation/infer_unet_subject_masks.py` -> `main(...)` | `subject_mask_runs/<run>` | Recommended unified subject-mask U-Net path. Defaults to probability-first `mask_probs_roi`, async output, and no progress repaint; `masks_roi` is an optional compatibility cache via `--write-masks-roi`. Currently supports `label_schema_id = "subject_v1_union"` with `mask_labels = ["subject_body", "eyes_union", "swim_bladder"]`. Partially supervised checkpoints should still write the canonical schema, but channels without semantic support should be marked unavailable and emitted as zero placeholders. |
