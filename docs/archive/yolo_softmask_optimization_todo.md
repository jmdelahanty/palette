# YOLO Soft-Mask Optimization TODO

Archived deferred optimization notes for the YOLO soft-mask path. This is not
part of the active schema/contract docs.
...
### 1. Eliminate duplicate matmul safely
...
- Requires verifying:
  ...
  - Any Ultralytics post-processing (e.g. NMS or Retina pipeline) is not broken.
- Expected gain: remove one full `coeffs @ protos + upsample` pass per ROI.

### 2. Optional: Full patch of `process_mask_native`
...
- Compare:
  ...
  - current full path
  - cache-only
  - cache + native upsample=False after safe interception
