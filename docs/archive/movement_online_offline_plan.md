...
- **Online**: detection/keypoint driven tracks (existing `track_kinematics_runs` pipeline).
- **Offline**: synthetic tracks derived from `analysis/chaser_fish_metrics` (via the new bundle).

Both products currently land in the flat `analysis/track_kinematics_runs/` namespace differentiated only by naming conventions and per-run metadata. The next step is to formalise two subtrees so consumers can browse predictable slots and the CLI can surface truly separate `latest` pointers.
...
- `analysis/track_kinematics_runs/online` and `.../offline` are required groups.
...
- The top-level `analysis/track_kinematics_runs` parent no longer stores run groups directly; it only contains the two children and optional compatibility pointers.
...
3. **plot_track_kinematics.py**
   - Update discovery logic to enumerate `analysis/track_kinematics_runs/online` and `.../offline`.
...
4. **Other Consumers**
   - Audit any scripts/notebooks referencing `analysis/track_kinematics_runs/<run>` directly (search for `track_kinematics_runs/`). Patch them to use the new helper.
   - Update docs (README, CLI help, pipeline diagrams) to showcase the new layout.

5. **Testing / Validation**
   - Unit tests (or integration scripts) that execute `track_kinematics` with both online and offline enabled, then assert runs land in the correct subgroup and `latest` pointers update correctly.
   - Run `plot_track_kinematics` on a freshly generated archive to confirm both plots appear by default.
...
1. Update `track_kinematics.py` to create runs inside `online/` and `offline/` subgroups.
2. Patch `plot_track_kinematics.py` (and any other tooling) to list the new structure.
