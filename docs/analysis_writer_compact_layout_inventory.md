...
| `refined_subject_masks_runs` | Logical mask store (`masks_roi`, compact `mask_bitpacked`, and/or compact `mask_rle`) plus component-local metrics/QC/review groups | Logical resolver implemented; dense masks are a compatibility/training materialization surface, not the only canonical physical store | Medium | Route readers through `MaskStore`. Future layout should stack common component metrics/QC as `(row, component)` arrays and reserve component groups for true component-specific authoring state. Keep dense `masks_roi` only where editing, review, or training materialization needs it. |
