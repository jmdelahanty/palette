...
`analytics_storage_schema_matrix.md` and
`stable_identity_incremental_materialization_decision.md`.
...
| --- | --- | --- | --- | --- |
| `analysis/swim_bout_runs` | Compact-v2 tabular layout by default; hierarchical v1 remains explicit compatibility | Compact-v2 default as of 2026-05-11; schema-8 authoritative frame-axis references as of 2026-07-17 | High, accepted | Keep resolver-first policy. New accepted runs use `compact_v2` and reference the exact source track frame axis; use `--frame-axis-storage embedded` for a portable fallback or `--layout hierarchical_v1` only for legacy/debug compatibility. |
...
| `analysis/eye_angle_runs` | Compact-dense-v2 dense channel tables by default; hierarchical v1 remains explicit compatibility | Compact-dense-v2 default as of 2026-05-11 | Medium-high, accepted | Keep resolver-first policy. New accepted runs use `compact_dense_v2`; use `--layout hierarchical_v1` only for legacy/debug compatibility. See `eye_angle_compact_v2_design.md`. |
...
| `refined_subject_masks_runs` | Dense authoritative `masks_roi` plus optional derived `mask_bitpacked`/`mask_rle` caches and component-local metrics/QC/review groups | Logical resolver implemented; modern editable runs require dense `masks_roi` | Medium | Route readers through `MaskStore`. Future layout may stack common component metrics/QC as `(row, component)` arrays while retaining dense `masks_roi` as the edit/training authority and reserving component groups for true component-specific authoring state. |
...
- `analysis/eye_angle_runs`
  ...
  - The loader also understands the compact-dense-v2 read surface:
    `roi_angles`, `frame_angles`, vector/QA dense channel arrays, and
    channel-index groups.
  - `eye_angle_analysis.py` now defaults to `--layout compact_dense_v2`.
  ...
  - Default-layout smoke generated without passing `--layout` on 2026-05-11:
    `eye_angle_default_compact_smoke_20260511` wrote
    `layout = compact_dense_v2`, resolved required frame-gaze channels, and
    passed strict JSON validation.
