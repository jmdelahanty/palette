...
Interpretation correction (2026-09-06): the linked September 1 results are
exploratory, recording-level, and not adjusted for missing subject/batch
clustering. Pre-training avoidance does not alone establish innateness;
passive-coping is a mechanistic hypothesis, not a validated state label. The
first three post-training minutes remain a candidate window rather than a
standard readout. Preserve the reported values while testing those hypotheses.
Historical implementation/test claims below are not current merge-readiness
evidence; see the [audit reconciliation](review_docs_rules_landing_handoff_2026-09-05.md#september-6-audit-reconciliation).
