...
def test_physical_v71_registry_upgrades_to_v72(tmp_path: Path) -> None:
    ...
    with sqlite3.connect(registry_path) as conn:
        ...
        conn.execute("DELETE FROM schema_version WHERE version = 72;")
        conn.execute("PRAGMA user_version = 71;")
        conn.commit()

    upgraded = Registry(registry_path)
    try:
        assert upgraded._current_schema_version() == 72
        tables = {
            str(row[0])
...
def test_migration72_rejects_preexisting_authority_tables(tmp_path: Path) -> None:
