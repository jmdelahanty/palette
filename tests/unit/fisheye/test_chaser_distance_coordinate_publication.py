...
from tests.persistent_fixture_cache import (
    PersistentFixture,
    persistent_directory_fixture,
)


_REPO_ROOT = Path(__file__).resolve().parents[3]
_CANONICAL_DETECTION_PATH = "detect_runs/canonical"
_CANONICAL_STIMULUS_RUN = "canonical_stimulus"
_CANONICAL_DISTANCE_PATH = "analysis/chaser_distance_runs/canonical_distance"
...
def _build_cached_canonical_source_archive(destination: Path) -> None:
    ...
    build_root.mkdir()
    zarr_path, detection_path, stimulus_run = _canonical_sources(build_root)
    if (
        detection_path != _CANONICAL_DETECTION_PATH
        or stimulus_run != _CANONICAL_STIMULUS_RUN
    ):
...
def _build_cached_published_canonical_archive(
    ...
    *,
    ...
    run_path = _publish_canonical_from_sources(
        destination,
        _CANONICAL_DETECTION_PATH,
        _CANONICAL_STIMULUS_RUN,
    )
    if run_path != _CANONICAL_DISTANCE_PATH:
        raise ValueError(
            "Canonical chaser-distance publication path changed unexpectedly."
        )
...
def _resolve_canonical_source_fixture(
    *,
    cache_root: Path | None = None,
) -> PersistentFixture:
    return persistent_directory_fixture(
        namespace="canonical-chaser-distance-source-archive",
        ...
        build=_build_cached_canonical_source_archive,
        validate=_validate_cached_canonical_source_archive,
        cache_root=cache_root,
    )


def _resolve_published_canonical_fixture(
    source_fixture: PersistentFixture,
    *,
    cache_root: Path | None = None,
) -> PersistentFixture:
    return persistent_directory_fixture(
        namespace="canonical-chaser-distance-published-archive",
        ...
        build=lambda destination: _build_cached_published_canonical_archive(
            destination,
            source_template=source_fixture.path,
        ),
        validate=_validate_cached_published_canonical_archive,
        cache_root=cache_root,
    )


@pytest.fixture(scope="module")
def canonical_source_persistent_fixture() -> PersistentFixture:
    return _resolve_canonical_source_fixture()


@pytest.fixture(scope="module")
def canonical_source_template(
    canonical_source_persistent_fixture: PersistentFixture,
) -> tuple[Path, str, str]:
    return (
        canonical_source_persistent_fixture.path,
        _CANONICAL_DETECTION_PATH,
        _CANONICAL_STIMULUS_RUN,
    )
...
def published_canonical_template(
    ...
    fixture = _resolve_published_canonical_fixture(
        canonical_source_persistent_fixture,
    )
    return fixture.path, _CANONICAL_DISTANCE_PATH
