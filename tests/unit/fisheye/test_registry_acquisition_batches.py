...
def test_batch_entity_and_assignment_provenance_is_exact(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert batch.registry_schema_version == 67
        ...
        assert row["acquisition_batch_creation_registry_schema_version"] == 67
        assert row["acquisition_batch_assignment_registry_schema_version"] == 67
    finally:
        registry.close()
