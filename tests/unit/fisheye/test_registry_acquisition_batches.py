...
def test_batch_entity_and_assignment_provenance_is_exact(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert batch.registry_schema_version == 71
        ...
        assert assignment.registry_schema_version == 71
        ...
        assert row["acquisition_batch_creation_registry_schema_version"] == 71
        assert row["acquisition_batch_assignment_registry_schema_version"] == 71
    finally:
        registry.close()
...
def test_migration_preserves_existing_rows_as_unassigned(tmp_path: Path) -> None:
    ...
    try:
        ...
        assert reopened._current_schema_version() == 71
        assert (
