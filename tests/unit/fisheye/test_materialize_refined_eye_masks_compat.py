...
def test_apply_component_review_status_triggers_eye_compat_materialization(monkeypatch) -> None:
    ...
    def _fake_materialize(refined, *, updated_components=None) -> None:
        calls.append((refined.run_name, tuple(updated_components) if updated_components is not None else None))

    monkeypatch.setattr(review_mod, "_materialize_refined_eye_compat_if_needed", _fake_materialize)
    refined_parent = _FakeGroup(path="refined_subject_masks_runs")
    refined = refined_parent.create_group("refined_subject_masks_001")
    refined.attrs["mask_labels"] = ["eye_left", "eye_right"]
    refined.attrs["component_review_statuses"] = {}

    payload, run_payload = review_mod.apply_component_review_status(
        refined_parent,
    ...
    assert calls == [("refined_subject_masks_001", ("eye_left",))]


def test_materialize_refined_eye_compat_helper_skips_non_eye_component_updates(monkeypatch) -> None:
    calls: list[str] = []

    monkeypatch.setattr(
        compat_mod,
        "materialize_refined_eye_masks_compat",
        lambda _group, *, refined_subject_run, overwrite: calls.append(refined_subject_run),  # noqa: ARG005
    )
    refined_parent = _FakeGroup(path="refined_subject_masks_runs")
    refined_group = refined_parent.create_group("refined_subject_masks_001")
    refined = review_mod.RefinedSubjectMaskRun(
        run_name="refined_subject_masks_001",
        parent=refined_parent,
        group=refined_group,
        component_names=("eye_left", "eye_right"),
        component_to_index={"eye_left": 0, "eye_right": 1},
    )

    review_mod._materialize_refined_eye_compat_if_needed(  # noqa: SLF001
        refined,
        updated_components=("subject_body",),
    )
    assert calls == []

    updated_at = review_mod._finalize_refined_subject_apply(  # noqa: SLF001
        refined,
        updated_components=("eye_left",),
    )
    assert isinstance(updated_at, str)
    assert refined_group.attrs["updated_at_utc"] == updated_at
    assert refined_parent.attrs["latest"] == "refined_subject_masks_001"
    assert calls == ["refined_subject_masks_001"]
