"""Tests for merged keypoint-training Zarr validation."""
...
def _write_valid_merged_pose_zarr(path: Path, *, suppress_legacy_string_warning: bool = True) -> None:
    ...
    kp.attrs["source_crop_run"] = "merged_export_smoke"
...
def test_validate_merged_keypoint_training_zarr_passes(tmp_path: Path) -> None:
    zarr_path = tmp_path / "merged_pose_ok.zarr"
    _write_valid_merged_pose_zarr(zarr_path)

    summary = validate_merged_keypoint_training_zarr(
        zarr_path,
...
def test_validate_merged_keypoint_training_zarr_rejects_invalid_frame_indices(tmp_path: Path) -> None:
    zarr_path = tmp_path / "merged_pose_bad.zarr"
    _write_valid_merged_pose_zarr(zarr_path)
...
def test_validate_legacy_fixed_unicode_source_index_without_backfill(tmp_path: Path) -> None:
    zarr_path = tmp_path / "merged_pose_legacy_strings_ok.zarr"
    ...
    with warnings.catch_warnings(record=True) as caught:
        ...
        _write_valid_merged_pose_zarr(zarr_path, suppress_legacy_string_warning=False)
