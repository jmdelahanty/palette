"""Validation for published baseline strategy analytics runs."""
...
def _safe_component(value: object, *, label: str) -> str:
    ...
    if not text or Path(text).name != text or text in {".", ".."}:
        raise StrategyAnalyticsValidationError(f"invalid {label}: {value!r}")
    return text
...
def validate_strategy_analytics_run(
    ...
    """Validate the manifest, every declared part, and every row count."""
    ...
    try:
        ...
        raise StrategyAnalyticsValidationError(
            f"cannot read strategy manifest {manifest_path}: {exc}"
        ) from exc
    if not isinstance(payload, dict):
        raise StrategyAnalyticsValidationError("strategy manifest must contain an object")
    errors = []
    ...
    for raw_table in tables:
        ...
        for raw_part in declared_parts:
            ...
            if not expected.is_file():
                errors.append(f"{table_name}: missing part {expected}")
                continue
            try:
                parquet_file = pq.ParquetFile(expected)
            ...
            if metadata.get(b"palette.schema_version", b"").decode() != str(SCHEMA_VERSION):
                errors.append(f"{table_name}/{part_name}: invalid footer schema version")
            ...
            checked_parts += 1
        try:
            declared_rows = int(row_counts.get(table_name, -1))
        except (TypeError, ValueError):
            errors.append(f"{table_name}: invalid declared row count")
        else:
            if observed_rows != declared_rows:
                errors.append(
                    f"{table_name}: observed {observed_rows} rows, manifest declares {declared_rows}"
                )
    if errors:
        raise StrategyAnalyticsValidationError(
            f"baseline strategy run {run_id!r} failed validation:\n- "
            + "\n- ".join(errors)
        )
    return {
        "status": "valid",
