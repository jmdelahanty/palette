"""Persist GoodCopBadCop per-epoch speed, bout, IBI, and chaser summaries."""
...
@dataclass(frozen=True)
class GoodCopBadCopEpochBehaviorSummaryResult:
    zarr_path: str
    recording_id: str
    component_name: str
...
def _make_per_epoch_chaser(
    *,
    ...
    if "chasers" in run_group and "chaser_index" in run_group["chasers"]:
        chaser_indices = np.asarray(run_group["chasers/chaser_index"][:], dtype=np.int32).reshape(-1)
    else:
    ...
    for window_index, window in enumerate(windows):
        for chaser_col in range(n_chasers):
            chaser_index = int(chaser_indices[chaser_col]) if chaser_col < chaser_indices.shape[0] else int(chaser_col)
            out[row_idx] = (
                int(window.window_id),
                int(window_index),
                str(window.label).encode("utf-8", "ignore")[:95],
...
def build_goodcopbadcop_epoch_behavior_summary_result(
    zarr_path: Path | str,
    *,
...
) -> GoodCopBadCopEpochBehaviorSummaryResult:
    ...
    recording_id = str(attrs.get("recording_id") or root.attrs.get("recording_id") or archive.stem)
    return GoodCopBadCopEpochBehaviorSummaryResult(
...
def write_goodcopbadcop_epoch_behavior_summary_component(
    zarr_path: Path | str,
    result: GoodCopBadCopEpochBehaviorSummaryResult,
    *,
    ...
    if component_name in parent:
        if not overwrite:
            raise ValueError(
                "GoodCopBadCop epoch behavior summary component already exists: "
        ...
        json_attr_safe(
            {
                ...
                "provenance": {
                    "stage": METHOD,
                    "created_by": "fisheye.analysis.goodcopbadcop_epoch_behavior_summary",
                    "inputs": source_refs,
                    "parameters": parameters,
                },
            }
...
def run_for_zarr(
    ...
    *,
    ...
    return write_goodcopbadcop_epoch_behavior_summary_component(
        zarr_path,
        result,
        overwrite=overwrite,
    )
