"""Compute GoodCopBadCop object-relative CRA near-field avoidance metrics."""
...
METHOD_VERSION = "1"
COMPONENT_PARENT_NAME = "cra_near_field"
DEFAULT_COMPONENT_NAME = "object_relative_near_field_v1"
RADIAL_DENSITY_PNG_ARTIFACT_NAME = "cra_near_field_radial_density_png"
DISTANCE_CDF_PNG_ARTIFACT_NAME = "cra_near_field_distance_cdf_png"
SUMMARY_PNG_ARTIFACT_NAME = "cra_near_field_summary_png"
INTERACTIVE_ARTIFACT_NAME = "cra_near_field_interactive"
INTERACTIVE_RENDERER = "palette-goodcopbadcop-cra-near-field-v1"
INTERACTIVE_SPEC_SCHEMA_ID = "palette.goodcopbadcop.cra_near_field.interactive_spec.v1"
...
@dataclass(frozen=True)
class CRANearFieldResult:
    zarr_path: str
    recording_id: str
    component_name: str
    chaser_distance_run_name: str
    chaser_distance_run_path: str
    source_cra_primary_endpoint_component: str
    source_cra_primary_endpoint_path: str
    ...
    objects: tuple[CRANearFieldObject, ...]
    phases: tuple[CRANearFieldPhase, ...]
    approach_percentile_mm: np.ndarray
    approach_percentile_cdf_fraction: np.ndarray
    object_x_px: np.ndarray
    object_y_px: np.ndarray
    object_x_mm: np.ndarray
    object_y_mm: np.ndarray
    object_distance_to_arena_center_mm: np.ndarray
    object_distance_to_wall_mm: np.ndarray
    object_displacement_from_pre_mm: np.ndarray
...
def _resolve_cra_primary_endpoint_component(
    run_group: zarr.Group,
    *,
    chaser_distance_run_path: str,
    component_name: str,
) -> tuple[zarr.Group, str, str]:
    parent = run_group.get(CRA_PRIMARY_ENDPOINT_PARENT)
    if parent is None:
        raise ValueError(
            "CRA near-field requires an existing CRA primary endpoint component under "
            f"{chaser_distance_run_path}/{CRA_PRIMARY_ENDPOINT_PARENT}."
        )
    ...
    if status and status != "computed":
        raise ValueError(f"CRA primary endpoint component is not computed: {resolved} status={status!r}.")
    return component, resolved, f"{chaser_distance_run_path}/{CRA_PRIMARY_ENDPOINT_PARENT}/{resolved}"


def _read_objects(component: zarr.Group) -> tuple[CRANearFieldObject, ...]:
    if "objects" not in component:
        raise ValueError("CRA primary endpoint component lacks objects group.")
    group = component["objects"]
    required = ("object_index", "object_role_code", "raw_color_rgba", "raw_color_hex_bytes")
...
def _read_phases(component: zarr.Group, *, total_frames: int) -> tuple[CRANearFieldPhase, ...]:
    if "phases" not in component:
        raise ValueError("CRA primary endpoint component lacks phases group.")
    group = component["phases"]
    required = ("phase_index", "phase_label_bytes", "effective_start_frame", "effective_end_frame")
    missing = [name for name in required if name not in group]
    if missing:
        raise ValueError(f"CRA primary endpoint phases group missing array(s): {missing}")
    ...
    if missing_labels:
        raise ValueError(f"CRA primary endpoint component missing phase(s): {missing_labels}")
    return tuple(by_label[label] for label in PHASE_LABELS)


def _phase_slice(phase: CRANearFieldPhase, total_frames: int) -> slice:
...
def _role_index(objects: Sequence[CRANearFieldObject], role: str) -> int:
...
def _summary_scalar(component: zarr.Group, key: str) -> float | None:
    summary = component.get("summary")
    if isinstance(summary, zarr.Group) and key in summary:
        arr = np.asarray(summary[key][:], dtype=np.float64).reshape(-1)
        if arr.size:
            value = float(arr[0])
            return value if math.isfinite(value) else None
    attrs_summary = component.attrs.get("summary")
    if isinstance(attrs_summary, Mapping) and key in attrs_summary:
...
def _rectangle_annulus_area_mm2(
    *,
    ...
    if not all(math.isfinite(v) for v in (object_x_px, object_y_px, width_px, height_px, pixels_per_mm)):
    ...
    center_x_mm = float(object_x_px) / float(pixels_per_mm)
    center_y_mm = float(object_y_px) / float(pixels_per_mm)
...
def _available_annulus_area_mm2(
    *,
    ...
    if (
        ...
        or not all(math.isfinite(v) for v in (object_x_px, object_y_px, pixels_per_mm))
        or pixels_per_mm <= 0
    ):
        return np.full(edges.shape[0] - 1, np.nan, dtype=np.float32)
    step = max(0.05, float(grid_step_mm))
    ...
    object_x_mm = float(object_x_px) / float(pixels_per_mm)
    object_y_mm = float(object_y_px) / float(pixels_per_mm)
    ...
    if band_mm > 0.0:
        ...
        in_circle &= (xx - center_x_mm) ** 2 + (yy - center_y_mm) ** 2 <= core_radius_mm**2
    object_distance = np.sqrt((xx - object_x_mm) ** 2 + (yy - object_y_mm) ** 2)
    counts, _ = np.histogram(object_distance[in_circle], bins=edges)
    return (counts.astype(np.float64) * (step * step)).astype(np.float32)
...
def _object_distance_to_center_and_wall_mm(
    *,
    geometry: ArenaGeometry,
    object_x_px: float,
    object_y_px: float,
    ...
    if not all(math.isfinite(v) for v in (object_x_px, object_y_px, pixels_per_mm)) or pixels_per_mm <= 0:
        return math.nan, math.nan
    if geometry.shape == "circle" and geometry.center_x_px is not None and geometry.center_y_px is not None and geometry.radius_px is not None:
        center_distance_px = math.hypot(float(object_x_px) - float(geometry.center_x_px), float(object_y_px) - float(geometry.center_y_px))
        return center_distance_px / float(pixels_per_mm), (float(geometry.radius_px) - center_distance_px) / float(pixels_per_mm)
    center_x_px = float(geometry.width_px) / 2.0
    center_y_px = float(geometry.height_px) / 2.0
    center_distance_px = math.hypot(float(object_x_px) - center_x_px, float(object_y_px) - center_y_px)
    wall_distance_px = min(
        float(object_x_px),
        float(object_y_px),
        float(geometry.width_px) - float(object_x_px),
        float(geometry.height_px) - float(object_y_px),
    )
...
def _compute_near_field_arrays(
    *,
    objects: Sequence[CRANearFieldObject],
    phases: Sequence[CRANearFieldPhase],
    ...
    approach = np.full((n_phases, n_objects, n_percentiles), np.nan, dtype=np.float32)
    approach_cdf_fraction = np.full((n_phases, n_objects, n_percentiles), np.nan, dtype=np.float32)
    object_x = np.asarray(object_phase_x_px[:n_phases, :n_objects], dtype=np.float32)
    object_y = np.asarray(object_phase_y_px[:n_phases, :n_objects], dtype=np.float32)
    object_x_mm = object_x / float(pixels_per_mm)
    object_y_mm = object_y / float(pixels_per_mm)
    object_center_distance = np.full(shape, np.nan, dtype=np.float32)
    object_wall_distance = np.full(shape, np.nan, dtype=np.float32)
    object_displacement_from_pre = np.full(shape, np.nan, dtype=np.float32)
    ...
    radial_count = np.zeros((n_phases, n_objects, n_radial_bins), dtype=np.int64)
    radial_fraction = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_area = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_density = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_count_wall_excluded = np.zeros((n_phases, n_objects, n_radial_bins), dtype=np.int64)
    radial_fraction_wall_excluded = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_area_wall_excluded = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_density_wall_excluded = np.full((n_phases, n_objects, n_radial_bins), np.nan, dtype=np.float32)
    radial_wall_excluded_valid_count = np.zeros(shape, dtype=np.int64)
    cdf_fraction = np.full((n_phases, n_objects, n_thresholds), np.nan, dtype=np.float32)
    ...
    for p_idx, phase in enumerate(phases):
        ...
        for reference_idx, (_label, ref_x, ref_y) in enumerate(reference_points):
            ...
            if ref_values.size:
                ...
                for t_idx, threshold in enumerate(cdf_thresholds_mm):
                    reference_cdf_fraction[p_idx, reference_idx, t_idx] = float(np.count_nonzero(ref_values <= float(threshold))) / float(ref_values.shape[0])
            reference_radial_area[p_idx, reference_idx, :] = _available_annulus_area_mm2(
                object_x_px=float(ref_x),
                object_y_px=float(ref_y),
                geometry=geometry,
                pixels_per_mm=float(pixels_per_mm),
        ...
        for o_idx, col_idx in enumerate(object_columns):
            center_distance, wall_distance = _object_distance_to_center_and_wall_mm(
                geometry=geometry,
                object_x_px=float(object_x[p_idx, o_idx]),
                object_y_px=float(object_y[p_idx, o_idx]),
                pixels_per_mm=float(pixels_per_mm),
            )
            object_center_distance[p_idx, o_idx] = float(center_distance) if math.isfinite(center_distance) else np.nan
            object_wall_distance[p_idx, o_idx] = float(wall_distance) if math.isfinite(wall_distance) else np.nan
            if p_idx == 0:
                object_displacement_from_pre[p_idx, o_idx] = 0.0
            else:
                object_displacement_from_pre[p_idx, o_idx] = (
                    math.hypot(
                        float(object_x[p_idx, o_idx]) - float(object_x[0, o_idx]),
                        float(object_y[p_idx, o_idx]) - float(object_y[0, o_idx]),
                    )
                    / float(pixels_per_mm)
                )
            missing_frame_count[p_idx, o_idx] = int(max(0, phase_len - np.count_nonzero(phase_fish_valid)))
            dropout[p_idx, o_idx] = (
            ...
            x = float(object_phase_x_px[p_idx, o_idx])
            y = float(object_phase_y_px[p_idx, o_idx])
    ...
    arrays = {
        ...
        "object_x_px": object_x,
        "object_y_px": object_y,
        "object_x_mm": object_x_mm.astype(np.float32),
        "object_y_mm": object_y_mm.astype(np.float32),
        "object_distance_to_arena_center_mm": object_center_distance,
        "object_distance_to_wall_mm": object_wall_distance,
        "object_displacement_from_pre_mm": object_displacement_from_pre,
...
def _value(array: np.ndarray, phase_index: int, object_index: int) -> float | None:
...
def _build_summary(
    *,
    ...
    summary: dict[str, Any] = {
        ...
        "frac_tracking_dropout_post": _value(tracking_dropout_fraction, 1, aggressive),
        "thigmotaxis_frac_pre": float(thigmotaxis_fraction[0]) if thigmotaxis_fraction.shape[0] > 0 and math.isfinite(float(thigmotaxis_fraction[0])) else None,
        "thigmotaxis_frac_post": float(thigmotaxis_fraction[1]) if thigmotaxis_fraction.shape[0] > 1 and math.isfinite(float(thigmotaxis_fraction[1])) else None,
        "mean_speed_mm_s_pre": float(mean_speed_mm_s[0]) if mean_speed_mm_s.shape[0] > 0 and math.isfinite(float(mean_speed_mm_s[0])) else None,
        "mean_speed_mm_s_post": float(mean_speed_mm_s[1]) if mean_speed_mm_s.shape[0] > 1 and math.isfinite(float(mean_speed_mm_s[1])) else None,
    ...
    for p_idx, percentile in enumerate(np.asarray(percentile_values, dtype=np.float64).reshape(-1)):
    ...
    for source, stem in (
        ...
        (near_zone_entry_rate_per_min, "nearzone_entry_rate"),
    ):
...
def build_cra_near_field_result(
    zarr_path: Path,
    *,
    chaser_distance_run: str = "latest",
    cra_primary_endpoint_component: str = "latest",
    ...
    if coordinate_frame != "arena_relative_canvas_px":
        raise ValueError(f"CRA near-field requires coordinate_frame='arena_relative_canvas_px'; got {coordinate_frame!r}.")
    if coordinate_origin != "top_left_of_active_arena":
        raise ValueError(f"CRA near-field requires coordinate_origin='top_left_of_active_arena'; got {coordinate_origin!r}.")
    ...
    cra_component, cra_component_name, cra_component_path = _resolve_cra_primary_endpoint_component(
        run_group,
        chaser_distance_run_path=distance_run_path,
        component_name=cra_primary_endpoint_component,
    )
    ...
    if missing:
        raise ValueError(f"Chaser-distance run missing required CRA near-field array(s): {missing}")
    ...
    objects = _read_objects(cra_component)
    phases = _read_phases(cra_component, total_frames=total_frames)
    if "object_phase" not in cra_component:
        raise ValueError("CRA primary endpoint component lacks object_phase group.")
    object_phase = cra_component["object_phase"]
    if "object_x_px" not in object_phase or "object_y_px" not in object_phase:
        raise ValueError("CRA primary endpoint object_phase group lacks object_x_px/object_y_px.")
    object_x_px = np.asarray(object_phase["object_x_px"][:], dtype=np.float32)
    object_y_px = np.asarray(object_phase["object_y_px"][:], dtype=np.float32)
    ...
    arrays, diagnostics, compute_warnings = _compute_near_field_arrays(
        ...
        pixels_per_mm=float(pixels_per_mm),
        geometry=geometry,
        object_phase_x_px=object_x_px,
        object_phase_y_px=object_y_px,
    ...
    diagnostics.update(
        {
            "source_cra_primary_endpoint_schema_id": cra_component.attrs.get("schema_id"),
            "qc_warning_count": len(compute_warnings),
        }
    )
    return CRANearFieldResult(
        zarr_path=str(zarr_path),
        recording_id=recording_id,
        component_name=str(component_name),
        chaser_distance_run_name=distance_run_name,
        chaser_distance_run_path=distance_run_path,
        source_cra_primary_endpoint_component=cra_component_name,
        source_cra_primary_endpoint_path=cra_component_path,
        source_stimulus_run=source_refs.get("source_stimulus_run") or run_group.attrs.get("source_stimulus_run"),
        source_stimulus_path=source_refs.get("source_stimulus_path") or run_group.attrs.get("source_stimulus_path"),
        source_stimulus_epoch_run=source_refs.get("source_stimulus_epoch_run") or run_group.attrs.get("source_stimulus_epoch_run"),
        source_stimulus_epoch_path=source_refs.get("source_stimulus_epoch_path") or run_group.attrs.get("source_stimulus_epoch_path"),
        fps=float(fps),
        ...
        approach_percentile_mm=arrays["approach_percentile_mm"],
        approach_percentile_cdf_fraction=arrays["approach_percentile_cdf_fraction"],
        object_x_px=arrays["object_x_px"],
        object_y_px=arrays["object_y_px"],
        object_x_mm=arrays["object_x_mm"],
        object_y_mm=arrays["object_y_mm"],
        object_distance_to_arena_center_mm=arrays["object_distance_to_arena_center_mm"],
        object_distance_to_wall_mm=arrays["object_distance_to_wall_mm"],
        object_displacement_from_pre_mm=arrays["object_displacement_from_pre_mm"],
        near_zone_occupancy_fraction=arrays["near_zone_occupancy_fraction"],
...
def render_cra_near_field_radial_density_png(result: CRANearFieldResult, *, dpi: int = 150) -> bytes:
    fig, axes = plt.subplots(1, len(result.phases), figsize=(5.0 * len(result.phases), 4.4), sharey=True, constrained_layout=True)
    ...
    fig.suptitle(f"CRA near-field radial density: {result.recording_id}", fontsize=12)
    buf = BytesIO()
...
def render_cra_near_field_distance_cdf_png(result: CRANearFieldResult, *, dpi: int = 150) -> bytes:
    fig, axes = plt.subplots(1, len(result.phases), figsize=(5.0 * len(result.phases), 4.4), sharey=True, constrained_layout=True)
    axes_list = list(axes.ravel()) if hasattr(axes, "ravel") else [axes]
    for p_idx, (ax, phase) in enumerate(zip(axes_list, result.phases)):
        for o_idx, obj in enumerate(result.objects):
    ...
    fig.suptitle(f"CRA near-field distance CDF: {result.recording_id}", fontsize=12)
    buf = BytesIO()
    fig.savefig(buf, format="png", dpi=int(dpi))
    plt.close(fig)
    buf.seek(0)
...
def render_cra_near_field_summary_png(result: CRANearFieldResult, *, dpi: int = 150) -> bytes:
    ...
    for o_idx, obj in enumerate(result.objects):
        offset = (o_idx - (len(result.objects) - 1) / 2.0) * width
        axes_list[0].bar(
            x + offset,
    ...
    for ax in axes_list:
        ...
        ax.legend(fontsize=8)
    fig.suptitle(f"CRA near-field summary: {result.recording_id}", fontsize=12)
    buf = BytesIO()
    fig.savefig(buf, format="png", dpi=int(dpi))
    plt.close(fig)
    buf.seek(0)
...
def _interactive_spec(result: CRANearFieldResult, component_path: str) -> dict[str, Any]:
    return {
...
def _source_refs(result: CRANearFieldResult) -> dict[str, Any]:
    return {
        "source_chaser_distance_run": result.chaser_distance_run_name,
        "source_chaser_distance_path": result.chaser_distance_run_path,
        "source_cra_primary_endpoint_component": result.source_cra_primary_endpoint_component,
        "source_cra_primary_endpoint_path": result.source_cra_primary_endpoint_path,
...
def _parameters(result: CRANearFieldResult) -> dict[str, Any]:
    return {
        "r_zone_mm": result.r_zone_mm,
        "r_in_mm": result.r_in_mm,
        "r_out_mm": result.r_out_mm,
...
def write_cra_near_field_component(
    zarr_path: Path,
    result: CRANearFieldResult,
    *,
    ...
    if component_name in parent:
        if not overwrite:
            raise ValueError(f"CRA near-field component already exists: {result.chaser_distance_run_path}/{COMPONENT_PARENT_NAME}/{component_name}")
    ...
    _write_array(phases, "total_frame_count", np.asarray([phase.total_frame_count for phase in result.phases], dtype=np.int64))
    phases.attrs.update({"row_axis": "cra_near_field_phases"})

    per_object = component.require_group("per_object_phase")
    _write_array(per_object, "approach_percentile_mm", result.approach_percentile_mm)
    _write_array(per_object, "approach_percentile_cdf_fraction", result.approach_percentile_cdf_fraction)
    _write_array(per_object, "object_x_px", result.object_x_px)
    _write_array(per_object, "object_y_px", result.object_y_px)
    _write_array(per_object, "object_x_mm", result.object_x_mm)
    _write_array(per_object, "object_y_mm", result.object_y_mm)
    _write_array(per_object, "object_distance_to_arena_center_mm", result.object_distance_to_arena_center_mm)
    _write_array(per_object, "object_distance_to_wall_mm", result.object_distance_to_wall_mm)
    _write_array(per_object, "object_displacement_from_pre_mm", result.object_displacement_from_pre_mm)
    _write_array(per_object, "near_zone_occupancy_fraction", result.near_zone_occupancy_fraction)
    _write_array(per_object, "near_zone_occupancy_fraction_of_epoch", result.near_zone_occupancy_fraction_of_epoch)
    _write_array(per_object, "near_zone_dwell_s", result.near_zone_dwell_s)
    _write_array(per_object, "near_zone_density_per_mm2", result.near_zone_density_per_mm2)
    _write_array(per_object, "near_zone_available_area_mm2", result.near_zone_available_area_mm2)
    _write_array(per_object, "near_zone_entry_count", result.near_zone_entry_count)
    _write_array(per_object, "near_zone_entry_rate_per_min", result.near_zone_entry_rate_per_min)
    _write_array(per_object, "near_zone_visit_median_dwell_s", result.near_zone_visit_median_dwell_s)
    _write_array(per_object, "near_zone_visit_total_dwell_s", result.near_zone_visit_total_dwell_s)
    _write_array(per_object, "valid_distance_count", result.valid_distance_count)
    ...
    per_object.attrs.update(
        {
            "axis_order": {
                "approach_percentile_mm": ["phase", "object", "percentile"],
                "approach_percentile_cdf_fraction": ["phase", "object", "percentile"],
                "object_geometry_arrays": ["phase", "object"],
                "near_zone_arrays": ["phase", "object"],
            },
            ...
            "near_zone_occupancy_denominator": "valid fish-to-object distance frames in effective phase",
    ...
    radial.attrs.update(
        {
            "axis_order": ["phase", "object", "radial_bin"],
            "geometry_status": result.geometry_status,
    ...
    cdf.attrs.update({"axis_order": ["phase", "object", "threshold"], "threshold_unit": "mm"})
    ...
    controls.attrs.update(
        {
            ...
            "purpose": "non-object reference diagnostics for global spatial shifts",
        }
    )
    ...
    attrs = {
        ...
        "provenance": {
            "stage": "cra_near_field",
            "created_by": "fisheye.analysis.cra_near_field",
            "inputs": source_refs,
            "parameters": parameters,
        },
    }
    ...
    if write_png:
        ...
        for artifact_name, renderer, description in (
            (
                ...
                render_cra_near_field_radial_density_png,
                "GoodCopBadCop CRA near-field radial occupancy density.",
            ),
            (
                DISTANCE_CDF_PNG_ARTIFACT_NAME,
                render_cra_near_field_distance_cdf_png,
                "GoodCopBadCop CRA near-field distance CDF.",
            ),
            (
                SUMMARY_PNG_ARTIFACT_NAME,
                render_cra_near_field_summary_png,
                "GoodCopBadCop CRA near-field summary.",
            ),
        ):
            write_png_visualization_artifact(
                ...
                parameters=parameters,
                extra_attrs={
                    "cra_near_field_schema_id": SCHEMA_ID,
                    "component_path": component_path,
    ...
    if write_interactive_spec:
        ...
        write_interactive_plot_spec_artifact(
            ...
            spec,
            description="GoodCopBadCop CRA near-field interactive plot spec.",
            created_by="fisheye.analysis.cra_near_field",
            renderer=INTERACTIVE_RENDERER,
...
def _result_payload(result: CRANearFieldResult, *, applied_path: str | None) -> dict[str, Any]:
    return {
        "schema_id": SCHEMA_ID,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--cra-primary-endpoint-component", default="latest")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    result = build_cra_near_field_result(
        ...
        cra_primary_endpoint_component=str(args.cra_primary_endpoint_component),
    ...
    applied_path = None
    if args.apply:
        applied_path = write_cra_near_field_component(
            Path(args.zarr_path),
            result,
    ...
    if args.json:
        ...
        print(f"cra_primary_endpoint_component\t{result.source_cra_primary_endpoint_component}")
