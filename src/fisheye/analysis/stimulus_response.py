...
def parse_protocol_steps(
    ...
    *,
    ...
    event_names = events_group["event_name"][:]
    step_indices = events_group["step_index"][:].astype(np.int32)
    stimulus_modes = events_group["stimulus_mode"][:].astype(np.int32)
    camera_frames = events_group["camera_frame_id"][:].astype(np.int64)

    # Decode event_name if stored as bytes.
    if event_names.dtype.kind in ("S", "O"):
        decoded = []
        ...
        event_names = np.array(decoded, dtype=object)
...
def resolve_grating_direction(
    ...
    and applies the camera-to-projector angular offset.
    """
    params = step.stimulus_params
    direction = (
        params.get("orientation_degrees")
        or params.get("angle_degrees")
        or params.get("grating_orientation")
        or 0.0
    )
    return float(direction) + offset_deg
...
def reconstruct_loom_trials(
    ...
    """
    loom_duration_s = float(step.stimulus_params.get("loom_duration_sec", 0.0))
...
def main(argv: Optional[Iterable[str]] = None) -> None:
    ...
    for step in steps:
        if step.stimulus_mode == _CONCENTRIC_GRATING:
            center = resolve_concentric_center_mm(root, step.stimulus_params)
    ...
    def _compute_one_step(step: ProtocolStep) -> Dict[str, Any]:
        ...
        if step.stimulus_mode == _MOVING_GRATING:
            ...
            grating_speed = float(step.stimulus_params.get("grating_speed_mm_s", 0.0))
            ...
            result["grating"] = GratingStepData(per_frame=pf, per_fish=gpf, time_series=ts)
