...
def _first_array_length_in_group(group: Any | None, names: tuple[str, ...]) -> int:
    if group is None:
        return 0
    for name in names:
        try:
...
def load_subject_shape_run_tables(
    ...
    *,
    ...
    body_frame = _read_array_mapping(
        body_frame_group,
    ...
    source_refined_subject_masks = _read_array_mapping(
        source_revision_group,
