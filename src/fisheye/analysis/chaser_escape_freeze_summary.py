"""Compute a chaser-centric escape and freeze summary for a selected chaser."""
...
SCHEMA_VERSION = 1
METHOD = "chaser_centric_escape_freeze_summary"
METHOD_VERSION = "2"
COMPONENT_PARENT_NAME = "chaser_escape_freeze"
DEFAULT_COMPONENT_NAME = "chaser_0_escape_freeze_v1"
...
class ChaserEscapeFreezeSummaryResult:
    zarr_path: str
    recording_id: str
    component_name: str
...
def build_chaser_escape_freeze_summary_result(
    zarr_path: Path,
    *,
    ...
    summary = {
        ...
        "mean_response_speed_mm_s": (
            _finite_mean(metric_table["response_mean_speed_mm_s"])
            if metric_table.size
            else np.nan
        ),
        "median_net_displacement_mm": (
            float(np.nanmedian(metric_table["net_displacement_mm"]))
            if metric_table.size
            else np.nan
        ),
        "median_radial_excursion_mm": (
            float(np.nanmedian(metric_table["radial_excursion_mm"]))
            if metric_table.size
            else np.nan
        ),
        "mean_freeze_low_speed_fraction": (
            _finite_mean(metric_table["freeze_low_speed_fraction"])
            if metric_table.size
            else np.nan
        ),
        "interpretation": "candidate_escape_freeze_summary_for_selected_active_pursuit_chaser",
    }
    diagnostics = {
    ...
    return ChaserEscapeFreezeSummaryResult(
...
def render_per_trial_diagnostic_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
    ...
    if n_trials == 0:
        ...
        for row, trial in enumerate(trials):
            ...
            if rows.size:
                ax_xy.plot(
                    rows["fish_x_chaser_frame_mm"],
                    rows["fish_y_chaser_frame_mm"],
                    color="#2563eb",
                    linewidth=1.1,
                    alpha=0.85,
                )
        ...
        fig.suptitle(
            f"Chaser-centric escape/freeze summary: {result.recording_id}\n"
            f"Candidate labels from full-trial fish path; chaser heading points up; chaser {result.chaser_index}",
            fontsize=11,
        )
    buf = BytesIO()
...
def render_fish_centered_diagnostic_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
    trials = result.trial_table
    traj = result.trajectory_table
...
def render_speed_displacement_scatter_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
...
def render_response_class_bar_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
    metrics = result.metric_table
...
def render_trial_outcome_timeline_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
    trials = result.trial_table
    metrics = result.metric_table
...
def _fish_centered_polar_samples(
    result: ChaserEscapeFreezeSummaryResult,
) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    traj = result.trajectory_table
...
def render_fish_centered_polar_approach_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
...
def render_fish_centered_polar_density_png(
    result: ChaserEscapeFreezeSummaryResult, *, dpi: int = 150
) -> bytes:
...
def write_chaser_escape_freeze_summary_component(
    zarr_path: Path,
    result: ChaserEscapeFreezeSummaryResult,
    *,
    ...
    if component_name in parent:
        if not overwrite:
            raise ValueError(
                f"Escape/freeze summary component already exists: "
    ...
    attrs = {
        ...
        "status": "complete",
        "analysis_scope": "candidate_escape_freeze_classifier",
        "classification_locked": False,
        "row_axis": "active_pursuit_trials",
        ...
        "interpretation_scope": (
            "candidate within-chaser response characterization; cross-role specificity "
            "requires a separate cohort-level comparison"
        ),
        "git_commit": git.get("commit_hash"),
        "git_branch": git.get("branch"),
        "git_dirty": git.get("is_dirty"),
        "provenance": {
            "stage": "chaser_escape_freeze_summary",
            "created_by": "fisheye.analysis.chaser_escape_freeze_summary",
            "inputs": source_refs,
            "parameters": parameters,
        },
    }
...
def _result_payload(
    result: ChaserEscapeFreezeSummaryResult, *, applied_path: str | None
) -> dict[str, Any]:
    return {
        "schema_id": SCHEMA_ID,
...
def build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "This labels candidate trials but does not mark the classifier as cohort-locked."
        ),
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    result = build_chaser_escape_freeze_summary_result(
        args.zarr_path,
    ...
    applied_path = None
    if args.apply:
        applied_path = write_chaser_escape_freeze_summary_component(
            args.zarr_path,
            result,
