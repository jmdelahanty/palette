#!/usr/bin/env python3
"""
Inspect the positional inputs that feed the 2D histogram in plot_movement.py.

For a given movement run/track, this reports how many samples are present,
how many contain finite coordinates, and the min/max X/Y values in both pixels
and millimetres (when those datasets exist). It also highlights which unit
plot_movement would use for the heatmap.
"""
...
from fisheye.analysis.plot_movement import pick_units, resolve_movement_run, resolve_track
...
def analyze_track_positions(
    ...
    run_name, run_group = resolve_movement_run(root, movement_run_name, console)
    ...
    console.log(f"[bold]plot_movement will use units: {unit_label}[/bold]")
    ...
    if pos_x_default is not None and pos_y_default is not None:
        console.log(
            f"[cyan]plot_movement heatmap input coverage:[/cyan] "
    ...
    console.log(
        f"[green]Inspection complete for movement run '{run_name}', track {track_id_resolved}.[/green]"
    )


def parse_args(argv: Optional[Sequence[str]] = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Inspect the positional inputs used by plot_movement.py for a specific movement track."
    )
