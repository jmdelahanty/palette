    ...
    if report.frame_metadata.status != "skip":
        lines.extend(
            [
                ...
                f"  expected_ratio: {report.frame_metadata.expected_ratio:.6f}",
                f"  expected_ratio_source: {report.frame_metadata.expected_ratio_source or '—'}",
                f"  inferred_camera_fps: {report.frame_metadata.inferred_camera_fps:.6f}" if report.frame_metadata.inferred_camera_fps is not None else "  inferred_camera_fps: —",
                f"  inferred_stimulus_fps: {report.frame_metadata.inferred_stimulus_fps:.6f}" if report.frame_metadata.inferred_stimulus_fps is not None else "  inferred_stimulus_fps: —",
                f"  mean_camera_frame_step: {report.frame_metadata.mean_camera_frame_step:.6f}" if report.frame_metadata.mean_camera_frame_step is not None else "  mean_camera_frame_step: —",
                f"  expected_camera_frame_step: {report.frame_metadata.expected_camera_frame_step:.6f}" if report.frame_metadata.expected_camera_frame_step is not None else "  expected_camera_frame_step: —",
