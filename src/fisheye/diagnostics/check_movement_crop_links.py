"""Inspect movement analysis runs and report their crop provenance.

This diagnostic walks the ``analysis/movement_runs`` tree and attempts to locate
the crop run referenced by each movement run.  It is useful when the
visualization tooling cannot resolve ROI imagery because the movement run does
not record which crop archive it depends on.

Example
-------
python -m fisheye.diagnostics.check_movement_crop_links data.zarr
python -m fisheye.diagnostics.check_movement_crop_links data.zarr --run offline/movement_20250101_120000
"""
...
def _extract_provenance(attrs: zarr.attrs.Attrs) -> Dict[str, Optional[str]]:
    """Extract crop, keypoint, and detection references from movement attrs."""
    crop_candidates = []
    keypoint_candidates = []
    ...
    if inputs:
        ...
        if "id_assignment_metadata" in inputs:
            meta = inputs["id_assignment_metadata"]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run",
        help="Specific movement run to inspect (accepts 'offline/<name>' or just the run name)",
    )
...
def main() -> None:
    ...
    if not isinstance(analysis, zarr.Group):
        raise ValueError("Archive has no 'analysis' group; run movement analysis first.")
    movement_root = analysis.get("movement_runs")
    if not isinstance(movement_root, zarr.Group):
        raise ValueError("Archive has no 'analysis/movement_runs' group.")

    runs = list(_iter_movement_runs(movement_root, args.run))
    if not runs:
        if args.run:
            raise ValueError(f"No movement runs matched selector '{args.run}'.")
        console.print("[yellow]No movement runs found under analysis/movement_runs.[/yellow]")
        return
    ...
    console.print(f"[bold]Movement crop provenance for[/bold] {args.zarr_path}")
    console.print(table)
