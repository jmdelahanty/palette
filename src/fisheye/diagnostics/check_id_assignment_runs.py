"""
Diagnostic for ID-assignment run health.

Validates each `id_assignment_runs/<run>` against the canonical stage array
...
ID_ARRAYS = array_specs_by_name(ID_ASSIGNMENT_SPEC)
REQUIRED_ARRAYS = [
...
def _check_id_assignment_runs(console: Console, parent: zarr.Group | None) -> None:
    if parent is None:
        console.print("[red]id_assignment_runs group not found in Zarr archive.[/red]")
        return

    run_names = sorted(name for name in parent.keys() if isinstance(parent[name], zarr.Group))
    if not run_names:
        console.print("[yellow]No id-assignment runs found.[/yellow]")
        return

    latest = parent.attrs.get("latest")
    console.print(f"[bold]id_assignment_runs[/bold] (latest: {latest or 'unknown'})")
    ...
    for run_name in run_names:
        ...
        result = validate_run(group, ID_ASSIGNMENT_SPEC)
        if result.errors:
        ...
        if "n_detections_per_mask" in group:
            details.append(f"n_detections_per_mask: {group['n_detections_per_mask'].shape}")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Diagnose id-assignment run metadata.")
    ...
    _check_id_assignment_runs(console, root.get("id_assignment_runs"))
