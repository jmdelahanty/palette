"""Fresh-process typed execution for compact-v3 stimulus-response candidates."""
...
from fisheye.analysis_workflows.materializers.stimulus_response import (
    STIMULUS_RESPONSE_EXECUTION_BINDING_ATTR,
    STIMULUS_RESPONSE_EXECUTION_PHASE_ORDER,
    materialize_stimulus_response_execution_candidate,
    tombstone_stimulus_response_execution_candidate,
)
...
RUNNER_REF = (
    "fisheye.diagnostics.stimulus_response_candidate_execution:"
    "execute_stimulus_response_candidate"
)
RUN_PARENT = "analysis/stimulus_response_runs"
_SELECTOR_SNAPSHOT_SCHEMA_ID = "palette.stimulus_response_candidate_selector_snapshot"
_SELECTOR_SNAPSHOT_SCHEMA_VERSION = 1


class StimulusResponseCandidateExecutionFailed(RuntimeError):
...
def stimulus_response_selector_snapshot_sha256(
    archive_path: str | Path,
) -> str:
    """Digest the complete selector-owning stimulus-response parent attrs."""
    ...
    return canonical_json_sha256(
        {
            ...
            "stage_id": STIMULUS_RESPONSE_EXECUTION_FAMILY_ID,
...
def _scientific_source_matches_invocation(
    source_identity: Mapping[str, Any],
    parameters: Mapping[str, Any],
) -> None:
    require_stimulus_response_invocation_parameters(parameters)
    source_parameters = source_identity.get("parameters")
    authorities = source_identity.get("source_authorities")
    if not isinstance(source_parameters, Mapping) or not isinstance(
        authorities,
        Mapping,
    ):
        raise ValueError("stimulus-response source identity is incomplete")
    scientific = parameters["scientific_parameters"]
    if any(source_parameters.get(name) != value for name, value in scientific.items()):
        raise ValueError("stimulus-response scientific parameters differ from source")
    expected = {
        "source_track_kinematics_scope": "offline",
        "source_track_kinematics_run": str(authorities["track_run_path"]).rsplit(
            "/", 1
        )[1],
        "source_track_motion_manifest_sha256": authorities[
            "track_motion_manifest_sha256"
        ],
        "source_stimulus_run": str(authorities["stimulus_run_path"]).rsplit("/", 1)[1],
        ...
        "source_swim_bout_run": (
            None
            if authorities["swim_bout_run_path"] is None
            else str(authorities["swim_bout_run_path"]).rsplit("/", 1)[1]
        ),
        "source_swim_bout_logical_tree_sha256": authorities[
            "swim_bout_logical_tree_sha256"
        ],
    }
    for field, value in expected.items():
        if parameters[field] != value:
            raise ValueError(f"stimulus-response invocation {field} differs")
    expected_bout_mode = (
        "disabled" if authorities["swim_bout_run_path"] is None else "explicit"
    )
    if parameters["source_bout_mode"] != expected_bout_mode:
        raise ValueError("stimulus-response bout mode differs from source")
...
def _source_preflight(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("stimulus-response source must be one explicit immutable run")
    source = root[source_path]
    identity = build_stimulus_response_source_identity(
        root,
        source_run_path=source_path,
    )
    _scientific_source_matches_invocation(identity, parameters)
    hashes = compute_stimulus_response_logical_hashes(source)
    receipt = build_stimulus_response_storage_receipt(
        arrays_by_path=stimulus_response_arrays(source),
    ...
    if receipt != dict(planned_storage_receipt):
        raise ValueError("live stimulus-response storage plan differs from suite")
    return {
        "source_group": source,
        ...
        "coordinate_evidence": build_stimulus_response_coordinate_evidence(identity),
    }
...
def _translate_phases(runtime: Mapping[str, Any]) -> list[CandidatePhaseMeasurement]:
    require_runtime_telemetry(
        runtime,
        expected_materializer="stimulus_response_execution_candidate",
        allowed_phase_order=STIMULUS_RESPONSE_EXECUTION_PHASE_ORDER,
    ...
    if not isinstance(records, list) or [record.get("name") for record in records] != [
        ...
        raise ValueError("stimulus-response materializer phase sequence differs")
    result: list[CandidatePhaseMeasurement] = []
    for phase, record in zip(expected, records, strict=True):
        if record.get("outcome") != "ok":
            raise ValueError("successful stimulus-response execution has failed phase")
        cpu = record.get("cpu_seconds")
        if not isinstance(cpu, Mapping):
            raise ValueError("stimulus-response phase CPU telemetry is invalid")
        result.append(
            CandidatePhaseMeasurement(
                phase=phase,
...
def require_stimulus_response_execution_attempt(
    value: Mapping[str, Any],
    *,
    expected_request_payload_digest: str,
) -> None:
    """Validate shared attempt-v2 plus stimulus-response runtime telemetry."""
    ...
    if (
        ...
        raise ValueError("stimulus-response execution-attempt envelope differs")
...
def execute_stimulus_response_candidate(
    request: Mapping[str, Any],
    *,
    driver_pid: int,
) -> dict[str, Any]:
    """Execute one digest-bound compact-v3 candidate in a direct child."""
    ...
    if (
        adapter["stage_id"] != STIMULUS_RESPONSE_EXECUTION_FAMILY_ID
        or adapter["computation_mode"] != "scientific_compute"
        or adapter["coordinate_contract_status"]
        != "bound_source_validation_implemented"
        or invocation["contract_id"] != "stimulus_response_v1"
    ):
        raise ValueError("stimulus-response runner received another typed contract")
    if type(driver_pid) is not int or driver_pid <= 0 or driver_pid != os.getppid():
        raise ValueError("stimulus-response execution must run in a direct child")
    ...
    parameters = require_stimulus_response_invocation_parameters(
    ...
    try:
        if stimulus_response_selector_snapshot_sha256(archive) != selector_before:
            raise ValueError("live stimulus-response selector differs from request")
        ...
        if source["decoded_identity_sha256"] != payload["source_identity_sha256"]:
            raise ValueError("stimulus-response decoded identity differs from request")
        source_hashes = source["logical_hashes"]
        if not isinstance(source_hashes, Mapping):
            raise ValueError("stimulus-response source hashes are invalid")
        ...
        def accept_published_candidate(
            ...
            if (
                candidate.attrs.get(STIMULUS_RESPONSE_EXECUTION_BINDING_ATTR)
                != execution_binding
                or candidate.attrs.get("source_staging_mode")
                != STIMULUS_RESPONSE_SOURCE_STAGING_MODE
            ):
                raise ValueError(
                    "published stimulus-response execution binding differs"
                )
            ...
            if (
                ...
                or fresh["coordinate_evidence"] != source["coordinate_evidence"]
            ):
                raise ValueError("stimulus-response source changed during publication")
            if candidate.attrs.get(STIMULUS_RESPONSE_STORAGE_PLAN_RECEIPT_ATTR) != (
                dict(planned_receipt)
            ):
                raise ValueError("published stimulus-response storage plan differs")
            require_stimulus_response_candidate_scientific_binding(
                candidate,
                source_identity=source["source_identity_document"],
            )
            if compute_stimulus_response_logical_hashes(candidate) != dict(
                source_hashes
            ):
                raise ValueError("published stimulus-response decoded values differ")
            selector_after = stimulus_response_selector_snapshot_sha256(archive)
            if selector_after != selector_before:
                raise ValueError("stimulus-response execution changed selector state")
            return {
                "coordinate_evidence": source["coordinate_evidence"],
        ...
        materialized = materialize_stimulus_response_execution_candidate(
            archive,
            ...
            expected_source_identity_sha256=str(source["source_identity_sha256"]),
        ...
        if (
            ...
            raise ValueError("atomic stimulus-response acceptance evidence differs")
        ...
        attempt = _failed_attempt(
            ...
            selector_after_sha256=stimulus_response_selector_snapshot_sha256(archive),
        ...
        raise StimulusResponseCandidateExecutionFailed(
            f"Stimulus-response execution failed during {failure_phase}: {exc}",
            attempt=attempt,
        ) from exc


def run_stimulus_response_candidate_fresh_process(
    ...
    *,
    ...
    if (
        ...
        != protected["production_profiles_sha256"]
        or stimulus_response_selector_snapshot_sha256(archive)
    ...
    errors: dict[str, str] = {}
    observers = {
        "selector_after_sha256": lambda: stimulus_response_selector_snapshot_sha256(
            archive
        ),
    ...
    try:
        ...
        child_error: BaseException = RuntimeError(
            "fresh stimulus-response child exited nonzero: "
        ...
        raise StimulusResponseCandidateExecutionFailed(
            "Fresh stimulus-response child failed; see immutable attempt record: "
            f"{detail}",
            attempt=attempt,
        )
    finally:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        receipt = execute_stimulus_response_candidate(
            request,
            driver_pid=args.driver_pid,
        )
    except StimulusResponseCandidateExecutionFailed as exc:
