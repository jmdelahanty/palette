"""Fresh-process typed execution for primitive-v2 stimulus-epoch candidates."""
...
RUNNER_REF = (
    "fisheye.diagnostics.stimulus_epoch_candidate_execution:"
    "execute_stimulus_epoch_candidate"
)
...
class StimulusEpochCandidateExecutionFailed(RuntimeError):
...
def stimulus_epoch_selector_snapshot_sha256(archive_path: str | Path) -> str:
    """Digest the complete selector-owning stimulus-epoch parent surface."""
    ...
    payload = {
        ...
        "stage_id": STIMULUS_EPOCH_EXECUTION_FAMILY_ID,
...
def _source_preflight(
    ...
    source_path: str,
    *,
    expected_stimulus_fingerprint: str,
    expected_epoch_lineage_hash: str,
    ...
    if (
        ...
        raise ValueError("stimulus-epoch source must be one explicit immutable run")
    source = root[source_path]
    attrs = _attrs(source)
    errors = validate_legacy_stimulus_epoch_source(source)
    if errors:
        raise ValueError("stimulus-epoch source is invalid: " + "; ".join(errors))
    if (
        attrs.get("palette_run_completion_status") != "complete"
        or attrs.get("stage_selector_eligible") is not True
    ):
        raise ValueError(
            "stimulus-epoch source must be explicitly complete and eligible"
        )
    source_stimulus_path = attrs.get("source_stimulus_path")
    if type(source_stimulus_path) is not str:
        raise ValueError("stimulus-epoch source stimulus path is absent")
    stimulus = root[source_stimulus_path]
    stimulus_fingerprint = compute_stimulus_group_logical_fingerprint(stimulus)
    if stimulus_fingerprint != expected_stimulus_fingerprint:
        raise ValueError("live stimulus group fingerprint differs from the invocation")
    source_identity_document = build_stimulus_epoch_source_identity(
        source,
        source_stimulus_fingerprint=stimulus_fingerprint,
    )
    if source_identity_document["source_epoch_lineage_hash"] != (
        expected_epoch_lineage_hash
    ):
        raise ValueError("live stimulus-epoch lineage differs from the invocation")
    source_hashes = compute_stimulus_epoch_logical_hashes(source)
    declarations = build_stimulus_epoch_array_declarations(
        source,
        byte_planner_adopted=True,
    )
    storage_receipt = build_exact_tabular_storage_receipt(
        source,
        declarations=declarations,
        profile=get_storage_profile(STIMULUS_EPOCH_EXECUTION_PROFILE_ID),
    ).as_manifest()
    if storage_receipt != dict(planned_storage_receipt):
        raise ValueError("live stimulus-epoch storage plan differs from the suite")
    return {
        "source_group": source,
        "source_stimulus_path": source_stimulus_path,
        "stimulus_fingerprint": stimulus_fingerprint,
        "source_identity_document": source_identity_document,
        "source_identity_sha256": canonical_json_sha256(source_identity_document),
        "decoded_identity_sha256": canonical_json_sha256(source_hashes),
        "logical_hashes": source_hashes,
        "storage_receipt": storage_receipt,
        "coordinate_evidence": build_stimulus_epoch_temporal_axis_evidence(
...
def _validate_published_candidate(
    ...
    *,
    ...
    errors.extend(validate_stimulus_epoch_candidate_lineage(candidate))
    errors.extend(validate_stimulus_epoch_run_manifest(candidate))
    declarations = build_stimulus_epoch_array_declarations(
        candidate,
        byte_planner_adopted=True,
    )
    ...
    if (
        attrs.get("schema_id") != STIMULUS_EPOCH_RUN_SCHEMA_ID
        or attrs.get("schema_version") != STIMULUS_EPOCH_RUN_SCHEMA_VERSION
        ...
        errors.append("published candidate state or schema differs")
    if attrs.get(ANALYSIS_STORAGE_PLAN_RECEIPT_ATTR) != dict(expected_storage_receipt):
        errors.append("published candidate storage receipt differs")
    if compute_stimulus_epoch_logical_hashes(candidate) != dict(expected_hashes):
        errors.append("published candidate decoded values differ from source")
    if errors:
        raise ValueError(
            "published stimulus-epoch candidate validation failed: " + "; ".join(errors)
        )
...
def _translate_phases(runtime: Mapping[str, Any]) -> list[CandidatePhaseMeasurement]:
    require_runtime_telemetry(
        runtime,
        expected_materializer="stimulus_epoch_candidate",
        allowed_phase_order=STIMULUS_EPOCH_EXECUTION_PHASE_ORDER,
    ...
    if not isinstance(records, list) or [record.get("name") for record in records] != [
        ...
        raise ValueError("stimulus-epoch materializer phase sequence differs")
    result: list[CandidatePhaseMeasurement] = []
    for phase, record in zip(expected, records, strict=True):
        if record.get("outcome") != "ok":
            raise ValueError(
                "successful stimulus-epoch materializer has a failed phase"
            )
        cpu = record.get("cpu_seconds")
        if not isinstance(cpu, Mapping):
            raise ValueError("stimulus-epoch phase CPU telemetry is invalid")
        result.append(
            CandidatePhaseMeasurement(
                phase=phase,
...
def require_stimulus_epoch_execution_attempt(
    value: Mapping[str, Any],
    *,
    expected_request_payload_digest: str,
) -> None:
    """Validate shared attempt-v2 plus stimulus-specific telemetry."""
    ...
    if (
        ...
        raise ValueError("stimulus-epoch execution-attempt envelope differs")
...
def execute_stimulus_epoch_candidate(
    request: Mapping[str, Any],
    *,
    driver_pid: int,
) -> dict[str, Any]:
    """Execute one digest-bound primitive-v2 candidate in a direct child."""
    ...
    if (
        adapter["stage_id"] != STIMULUS_EPOCH_EXECUTION_FAMILY_ID
        or adapter["computation_mode"] != "logical_rematerialization"
        or adapter["coordinate_contract_status"] != "temporal_axis_implemented"
        or invocation["contract_id"] != "stimulus_epochs_v1"
    ):
        raise ValueError("stimulus-epoch runner received another typed contract")
    if type(driver_pid) is not int or driver_pid <= 0 or driver_pid != os.getppid():
        raise ValueError("stimulus-epoch execution must run in a direct child")
    ...
    try:
        if stimulus_epoch_selector_snapshot_sha256(archive) != selector_before:
            raise ValueError("live stimulus-epoch selector differs from the request")
        commit, dirty = _git_identity()
        if commit != payload["palette_commit"]:
            raise ValueError("runner Git commit differs from the request")
        if parameters["storage_profile_id"] != STIMULUS_EPOCH_EXECUTION_PROFILE_ID:
            raise ValueError("stimulus-epoch invocation profile differs")
        root = open_zarr_root(archive, mode="r")
        source = _source_preflight(
            root,
            source_path,
            expected_stimulus_fingerprint=str(
                parameters["source_stimulus_fingerprint"]
            ),
            expected_epoch_lineage_hash=str(parameters["source_epoch_lineage_hash"]),
            planned_storage_receipt=planned_receipt,
        )
        source_identity = str(source["source_identity_sha256"])
        decoded_identity = str(source["decoded_identity_sha256"])
        if decoded_identity != payload["source_identity_sha256"]:
            raise ValueError(
                "stimulus-epoch decoded source identity differs from request"
            )
        source_hashes = source["logical_hashes"]
        if not isinstance(source_hashes, Mapping):
            raise ValueError("stimulus-epoch source hashes are invalid")
        coordinate = source["coordinate_evidence"]
        ...
        def accept_published_candidate(
            ...
            if candidate.attrs.get(EXECUTION_BINDING_ATTR) != execution_binding:
                raise ValueError("published stimulus-epoch execution binding differs")
            if candidate.attrs.get("source_staging_mode") != (
                "epoch_and_stimulus_logical_copy_v1"
            ):
                raise ValueError("published stimulus-epoch staging policy differs")
            fresh = _source_preflight(
                ...
                expected_epoch_lineage_hash=str(
                    parameters["source_epoch_lineage_hash"]
                ),
                planned_storage_receipt=planned_receipt,
            )
            if (
                fresh["source_identity_sha256"] != source_identity
                or fresh["decoded_identity_sha256"] != decoded_identity
                or fresh["logical_hashes"] != dict(source_hashes)
                or fresh["coordinate_evidence"] != coordinate
            ):
                raise ValueError("stimulus-epoch source changed during publication")
            _validate_published_candidate(
                candidate,
                expected_hashes=source_hashes,
                expected_storage_receipt=planned_receipt,
            )
            selector_after = stimulus_epoch_selector_snapshot_sha256(archive)
            if selector_after != selector_before:
                raise ValueError("stimulus-epoch execution changed selector state")
            return {
                "coordinate_evidence": coordinate,
                "selector_after_sha256": selector_after,
                "execution_binding": execution_binding,
                "source_identity_sha256": source_identity,
                "decoded_identity_sha256": decoded_identity,
                "storage_plan_receipt_sha256": canonical_json_sha256(planned_receipt),
            }

        failure_phase = "materializer"
        materialized = materialize_stimulus_epoch_candidate(
            archive,
            source_run=source_path.rsplit("/", 1)[1],
            run_name=candidate_name,
            scratch_root=payload["scratch_root"],
            profile_id=str(parameters["storage_profile_id"]),
        ...
        if (
            ...
            or acceptance.get("coordinate_evidence") != coordinate
            or acceptance.get("selector_after_sha256") != selector_before
            or acceptance.get("source_identity_sha256") != source_identity
            or acceptance.get("decoded_identity_sha256") != decoded_identity
        ):
            raise ValueError("atomic stimulus-epoch acceptance evidence differs")
        receipt = build_candidate_execution_receipt(
            ...
            logical_equality={
                ...
                "compared_array_count": STIMULUS_EPOCH_EXECUTION_ARRAY_COUNT,
        ...
        attempt = _failed_attempt(
            ...
            selector_after_sha256=stimulus_epoch_selector_snapshot_sha256(archive),
        ...
        raise StimulusEpochCandidateExecutionFailed(
            f"Stimulus-epoch execution failed during {failure_phase}: {exc}",
            attempt=attempt,
        ) from exc


def run_stimulus_epoch_candidate_fresh_process(
    ...
    *,
    ...
    if (
        ...
        != protected["production_profiles_sha256"]
        or stimulus_epoch_selector_snapshot_sha256(archive)
    ...
    errors: dict[str, str] = {}
    observers = {
        "selector_after_sha256": lambda: stimulus_epoch_selector_snapshot_sha256(
            archive
        ),
    ...
    try:
        if process.returncode == 0:
            if not hidden_receipt.is_file() or hidden_attempt.exists():
                ...
                raise StimulusEpochCandidateExecutionFailed(str(error), attempt=attempt)
            try:
        ...
        child_error: BaseException = RuntimeError(
            "fresh stimulus-epoch child exited nonzero: "
        ...
        raise StimulusEpochCandidateExecutionFailed(
            "Fresh stimulus-epoch child failed; see immutable attempt record: "
            f"{detail}",
            attempt=attempt,
        )
    finally:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        receipt = execute_stimulus_epoch_candidate(
            request,
            driver_pid=args.driver_pid,
        )
    except StimulusEpochCandidateExecutionFailed as exc:
