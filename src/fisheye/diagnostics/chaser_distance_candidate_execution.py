"""Fresh-process typed execution for primitive-v2 chaser-distance candidates."""
...
RUNNER_REF = (
    "fisheye.diagnostics.chaser_distance_candidate_execution:"
    "execute_chaser_distance_candidate"
)
SOURCE_RUN_PARENT = "analysis/chaser_distance_runs"
CANDIDATE_RUN_PARENT = "analysis/chaser_distance_storage_candidates"
_SELECTOR_SNAPSHOT_SCHEMA_ID = "palette.analysis_candidate_selector_snapshot"
_SELECTOR_SNAPSHOT_SCHEMA_VERSION = 1


class ChaserDistanceCandidateExecutionFailed(RuntimeError):
...
def chaser_distance_selector_snapshot_sha256(archive_path: str | Path) -> str:
    """Digest the complete selector-owning chaser-distance parent surface."""

    root = open_zarr_root(Path(archive_path).expanduser().resolve(), mode="r")
    parent = root[SOURCE_RUN_PARENT]
    payload = {
        "schema_id": _SELECTOR_SNAPSHOT_SCHEMA_ID,
        "schema_version": _SELECTOR_SNAPSHOT_SCHEMA_VERSION,
        "stage_id": CHASER_DISTANCE_EXECUTION_FAMILY_ID,
...
def _source_preflight(
    ...
    *,
    ...
    if (
        not source_path.startswith(f"{SOURCE_RUN_PARENT}/")
        or source_path.count("/") != 2
        or source_path.rsplit("/", 1)[1] in {"latest", "latest_complete"}
    ):
        raise ValueError("chaser-distance source must be one explicit immutable run")
    source = root[source_path]
    bound = load_bound_chaser_distance_run(root, source_path)
    source_binding = build_source_authority_binding(bound, source_group=source)
    if canonical_json_sha256(source_binding) != expected_source_binding_sha256:
        raise ValueError(
            "live chaser-distance source authority binding differs from invocation"
        )
    declarations = build_chaser_distance_base_declarations(source)
    source_hashes = base_logical_hashes(source, declarations)
    storage_receipt = build_base_storage_receipt(
        source,
        profile=get_storage_profile(CHASER_DISTANCE_EXECUTION_PROFILE_ID),
    ).as_manifest()
    if storage_receipt != dict(planned_storage_receipt):
        raise ValueError("live chaser-distance storage plan differs from the suite")
    decoded_identity = chaser_distance_decoded_identity_sha256(source)
    if decoded_identity != canonical_json_sha256(source_hashes):
        raise ValueError(
            "chaser-distance decoded identity helper differs from logical hashes"
        )
    return {
        "source_group": source,
        ...
        "coordinate_evidence": build_chaser_distance_source_preservation_evidence(
            source_binding
        ),
    }
...
def _validate_published_candidate(
    ...
    *,
    ...
    if (
        attrs.get("schema_id") != CHASER_DISTANCE_BASE_CANDIDATE_SCHEMA_ID
        or attrs.get("schema_version") != CHASER_DISTANCE_BASE_CANDIDATE_SCHEMA_VERSION
    ...
    if base_logical_hashes(candidate) != dict(expected_hashes):
        errors.append("published candidate decoded values differ from source")
    if errors:
        raise ValueError(
            "published chaser-distance candidate validation failed: "
            + "; ".join(errors)
        )
...
def _translate_phases(runtime: Mapping[str, Any]) -> list[CandidatePhaseMeasurement]:
    require_runtime_telemetry(
        runtime,
        expected_materializer="chaser_distance_base_candidate",
        allowed_phase_order=CHASER_DISTANCE_EXECUTION_PHASE_ORDER,
    ...
    if not isinstance(records, list) or [record.get("name") for record in records] != [
        ...
        raise ValueError("chaser-distance materializer phase sequence differs")
    result: list[CandidatePhaseMeasurement] = []
    for phase, record in zip(expected, records, strict=True):
        if record.get("outcome") != "ok":
            raise ValueError(
                "successful chaser-distance materializer has a failed phase"
            )
        cpu = record.get("cpu_seconds")
        if not isinstance(cpu, Mapping):
            raise ValueError("chaser-distance phase CPU telemetry is invalid")
        result.append(
            CandidatePhaseMeasurement(
                phase=phase,
...
def require_chaser_distance_execution_attempt(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("chaser-distance execution-attempt envelope differs")
...
def execute_chaser_distance_candidate(
    request: Mapping[str, Any],
    *,
    driver_pid: int,
) -> dict[str, Any]:
    ...
    if (
        adapter["stage_id"] != CHASER_DISTANCE_EXECUTION_FAMILY_ID
        or adapter["computation_mode"] != "logical_rematerialization"
        or adapter["coordinate_contract_status"] != "source_preservation_only"
        or invocation["contract_id"] != "chaser_distance_base_v1"
    ):
        raise ValueError("chaser-distance runner received another typed contract")
    if type(driver_pid) is not int or driver_pid <= 0 or driver_pid != os.getppid():
        raise ValueError("chaser-distance execution must run in a direct child")
    ...
    try:
        if chaser_distance_selector_snapshot_sha256(archive) != selector_before:
            raise ValueError("live chaser-distance selector differs from the request")
        commit, dirty = _git_identity()
        if commit != payload["palette_commit"]:
            raise ValueError("runner Git commit differs from the request")
        if parameters["storage_profile_id"] != CHASER_DISTANCE_EXECUTION_PROFILE_ID:
            raise ValueError("chaser-distance invocation profile differs")
        root = open_zarr_root(archive, mode="r")
        source = _source_preflight(
            root,
            source_path,
            expected_source_binding_sha256=str(
                parameters["source_authority_binding_sha256"]
            ),
        ...
        if decoded_identity != payload["source_identity_sha256"]:
            raise ValueError(
                "chaser-distance decoded source identity differs from request"
            )
        source_hashes = source["logical_hashes"]
        if not isinstance(source_hashes, Mapping):
            raise ValueError("chaser-distance source hashes are invalid")
        source_binding = source["source_binding"]
        if not isinstance(source_binding, Mapping):
            raise ValueError("chaser-distance source binding is invalid")
        ...
        def accept_published_candidate(
            ...
            if candidate.attrs.get(EXECUTION_BINDING_ATTR) != execution_binding:
                raise ValueError("published chaser-distance execution binding differs")
            if candidate.attrs.get("source_staging_mode") != (
                "sealed_base_logical_copy_v1"
            ):
                raise ValueError("published chaser-distance staging policy differs")
            ...
            if (
                fresh["decoded_identity_sha256"] != decoded_identity
                or fresh["logical_hashes"] != dict(source_hashes)
                or fresh["source_binding"] != dict(source_binding)
                or fresh["coordinate_evidence"] != coordinate
            ):
                raise ValueError("chaser-distance source changed during publication")
            _validate_published_candidate(
                candidate,
                source_group=fresh["source_group"],
                expected_source_binding=source_binding,
                expected_hashes=source_hashes,
                expected_storage_receipt=planned_receipt,
            )
            selector_after = chaser_distance_selector_snapshot_sha256(archive)
            if selector_after != selector_before:
                raise ValueError("chaser-distance execution changed selector state")
            return {
        ...
        if (
            ...
            or acceptance.get("source_binding_sha256")
            != canonical_json_sha256(source_binding)
            or acceptance.get("decoded_identity_sha256") != decoded_identity
        ):
            raise ValueError("atomic chaser-distance acceptance evidence differs")
        receipt = build_candidate_execution_receipt(
            ...
            logical_equality={
                ...
                "compared_array_count": CHASER_DISTANCE_EXECUTION_ARRAY_COUNT,
        ...
        attempt = _failed_attempt(
            ...
            selector_after_sha256=chaser_distance_selector_snapshot_sha256(archive),
        ...
        raise ChaserDistanceCandidateExecutionFailed(
            f"Chaser-distance execution failed during {failure_phase}: {exc}",
            attempt=attempt,
        ) from exc


def run_chaser_distance_candidate_fresh_process(
    ...
    *,
    ...
    if (
        ...
        != protected["production_profiles_sha256"]
        or chaser_distance_selector_snapshot_sha256(archive)
    ...
    errors: dict[str, str] = {}
    observers = {
        "selector_after_sha256": lambda: chaser_distance_selector_snapshot_sha256(
            archive
        ),
    ...
    try:
        ...
        child_error: BaseException = RuntimeError(
            "fresh chaser-distance child exited nonzero: "
        ...
        raise ChaserDistanceCandidateExecutionFailed(
            "Fresh chaser-distance child failed; see immutable attempt record: "
            f"{detail}",
            attempt=attempt,
        )
    finally:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        receipt = execute_chaser_distance_candidate(
            request,
            driver_pid=args.driver_pid,
        )
    except ChaserDistanceCandidateExecutionFailed as exc:
