"""Fresh-process typed execution for bout-classification v2 candidates."""
...
RUNNER_REF = (
    ...
    "execute_bout_classification_candidate"
)
...
class BoutClassificationCandidateExecutionFailed(RuntimeError):
    """Raised with one immutable failed-attempt envelope."""
...
def bout_classification_selector_snapshot_sha256(
    ...
    return canonical_json_sha256(
        {
            ...
            "stage_id": BOUT_CLASSIFICATION_EXECUTION_FAMILY_ID,
...
def require_bout_classification_execution_request(value: Mapping[str, Any]) -> None:
    if not isinstance(value, Mapping) or not isinstance(value.get("payload"), Mapping):
        raise ValueError("bout-classification request is not an envelope")
    ...
    if not isinstance(invocation, Mapping) or not isinstance(
        ...
        raise ValueError("bout-classification invocation is not an envelope")
    payload = invocation["payload"]
    if payload.get("contract_id") != BOUT_CLASSIFICATION_INVOCATION_CONTRACT_ID:
        raise ValueError("bout-classification invocation contract differs")
    require_bout_classification_invocation_parameters(payload.get("parameters"))
...
def _source_preflight(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("bout-classification source must be immutable and explicit")
    ...
    if (
        ...
        raise ValueError("bout-classification source must be complete and eligible")
    require_exact_bout_classification_run(source)
    scientific = build_bout_classification_scientific_identity(source)
    scientific_sha256 = canonical_json_sha256(scientific)
    if scientific_sha256 != parameters["source_scientific_identity_sha256"]:
        raise ValueError("bout-classification scientific identity differs")
    suite = build_bout_classification_execution_suite(
        source,
        seed=17,
        repetitions=1,
    )
    if suite["payload"]["storage_plan_receipt"] != dict(planned_storage_receipt):
        raise ValueError("bout-classification live dimensions differ from suite")
    return {
        "source_group": source,
        "scientific_identity": scientific,
        "scientific_identity_sha256": scientific_sha256,
        "logical_manifest_sha256": (
            bout_classification_logical_manifest_sha256(source)
        ),
    }
...
def _validate_published_candidate(
    ...
    *,
    ...
    if bout_classification_logical_manifest_sha256(candidate) != (
        bout_classification_logical_manifest_sha256(source)
    ):
        errors.append("candidate decoded arrays differ")
    if build_bout_classification_scientific_identity(candidate) != (
        build_bout_classification_scientific_identity(source)
    ):
        errors.append("candidate scientific identity differs")
    if errors:
        raise ValueError(
            "published bout-classification candidate failed: " + "; ".join(errors)
        )
...
def _translate_phases(runtime: Mapping[str, Any]) -> list[CandidatePhaseMeasurement]:
    require_runtime_telemetry(
        runtime,
        expected_materializer="bout_classification_candidate",
        allowed_phase_order=BOUT_CLASSIFICATION_EXECUTION_PHASE_ORDER,
    ...
    if not isinstance(records, list) or [record.get("name") for record in records] != [
        ...
        raise ValueError("bout-classification materializer phase sequence differs")
    result: list[CandidatePhaseMeasurement] = []
    for phase, record in zip(expected, records, strict=True):
        if record.get("outcome") != "ok":
            raise ValueError("successful materializer contains a failed phase")
        cpu = record.get("cpu_seconds")
        if not isinstance(cpu, Mapping):
            raise ValueError("bout-classification phase CPU telemetry differs")
        result.append(
            CandidatePhaseMeasurement(
                phase=phase,
...
def require_bout_classification_execution_attempt(
    ...
    *,
    ...
    if (
        ...
        raise ValueError("bout-classification execution-attempt envelope differs")
...
def execute_bout_classification_candidate(
    request: Mapping[str, Any],
    *,
    driver_pid: int,
) -> dict[str, Any]:
    """Execute one exact replay in a direct fresh child."""

    require_bout_classification_execution_request(request)
    payload = request["payload"]
    adapter = payload["adapter_manifest"]["payload"]
    invocation = payload["invocation"]["payload"]
    if (
        adapter["stage_id"] != BOUT_CLASSIFICATION_EXECUTION_FAMILY_ID
        or adapter["computation_mode"] != "logical_rematerialization"
        or invocation["contract_id"] != BOUT_CLASSIFICATION_INVOCATION_CONTRACT_ID
    ):
        raise ValueError("bout-classification runner received another adapter")
    if type(driver_pid) is not int or driver_pid <= 0 or driver_pid != os.getppid():
        raise ValueError("bout-classification execution must run in a direct child")
    ...
    try:
        if bout_classification_selector_snapshot_sha256(archive) != selector_before:
            raise ValueError("live bout-classification selector differs")
        ...
        if source_identity != payload["source_identity_sha256"]:
            raise ValueError("bout-classification logical identity differs")
        ...
        def accept(
            ...
            if candidate.attrs.get(EXECUTION_BINDING_ATTR) != binding:
                raise ValueError("published execution binding differs")
            ...
            if (
                fresh["logical_manifest_sha256"] != source_identity
                or fresh["scientific_identity"] != source["scientific_identity"]
            ):
                raise ValueError(
                    "bout-classification source changed during publication"
                )
            _validate_published_candidate(
                candidate,
                source=fresh["source_group"],
                expected_storage_receipt=planned_receipt,
            )
            coordinate = build_bout_classification_coordinate_evidence(
                fresh["source_group"], candidate
            )
            selector_after = bout_classification_selector_snapshot_sha256(archive)
            if selector_after != selector_before:
                raise ValueError("bout-classification execution changed selectors")
            return {
        ...
        materialized = materialize_bout_classification_candidate(
            archive,
        ...
        if (
            ...
            raise ValueError("atomic bout-classification acceptance differs")
        receipt = build_candidate_execution_receipt(
            ...
            logical_equality={
                ...
                "compared_array_count": BOUT_CLASSIFICATION_ARRAY_COUNT,
        ...
        attempt = _failed_attempt(
            ...
            selector_after_sha256=(
                bout_classification_selector_snapshot_sha256(archive)
            ),
        ...
        raise BoutClassificationCandidateExecutionFailed(
            f"Bout-classification execution failed during {failure_phase}: {exc}",
            attempt=attempt,
        ) from exc


def run_bout_classification_candidate_fresh_process(
    ...
    *,
    ...
    if (
        ...
        != protected["production_profiles_sha256"]
        or bout_classification_selector_snapshot_sha256(archive)
    ...
    try:
        ...
        child_error: BaseException = RuntimeError(
            "fresh bout-classification child exited nonzero: "
        ...
        raise BoutClassificationCandidateExecutionFailed(
            "Fresh bout-classification child failed; see attempt record.",
            attempt=attempt,
        )
    finally:
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    try:
        receipt = execute_bout_classification_candidate(
            request,
            driver_pid=args.driver_pid,
        )
    except BoutClassificationCandidateExecutionFailed as exc:
...
__all__ = [
    ...
    "execute_bout_classification_candidate",
    "require_bout_classification_execution_attempt",
    "require_bout_classification_execution_request",
