"""Byte-derived immutable storage plans for subject-mask quality snapshots."""
...
SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_ID = "palette.stage_storage.subject_mask_quality"
SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_VERSION = 1
_ARRAY_METADATA_OBJECTS_PER_ARRAY = 1
_STAGE_GROUP_METADATA_OBJECTS = 2


@dataclass(frozen=True)
class SubjectMaskQualityStorageRule:
    path: str
    ...
    def __post_init__(self) -> None:
        ...
        if not path:
            raise ValueError("Subject-mask quality storage path cannot be empty.")
...
SUBJECT_MASK_QUALITY_STORAGE_RULES = tuple(
    SubjectMaskQualityStorageRule(
        path=path,
        access=(
            AccessPattern.EAGER
...
@dataclass(frozen=True)
class SubjectMaskQualityStoragePlanEntry:
    rule: SubjectMaskQualityStorageRule
    plan: StoragePlan
...
@dataclass(frozen=True)
class SubjectMaskQualityStoragePlanSet:
    dimensions: SubjectMaskQualityDimensions
    profile: StorageProfile
    entries: tuple[SubjectMaskQualityStoragePlanEntry, ...]

    def __post_init__(self) -> None:
        expected = SUBJECT_MASK_QUALITY_SCHEMA_V1.binding_paths
        observed = tuple(entry.rule.path for entry in self.entries)
        if observed != expected:
            raise ValueError("Storage plans must match quality schema order exactly.")
        for entry in self.entries:
            ...
            if plan.access_pattern != entry.rule.access.value:
                raise ValueError("Storage access classification mismatch.")
            if plan.write_mode != WriteMode.IMMUTABLE.value:
                raise ValueError("Published subject-mask quality must be immutable.")
            if plan.shard_axes != (0,):
                raise ValueError("Subject-mask quality may shard only along rows.")
            self._require_complete_rows(entry)

    @staticmethod
    def _require_complete_rows(entry: SubjectMaskQualityStoragePlanEntry) -> None:
        plan = entry.plan
        chunk = plan.chunk_shape
        if chunk is None:
            raise ValueError("Subject-mask quality arrays cannot be scalar.")
        ...
        if plan.shard_shape is not None:
            ...
            if plan.write_ownership != "whole_shard_single_writer":
                raise ValueError("Sharded writes require whole-shard ownership.")
        elif plan.write_ownership != "single_writer_immutable_materialization":
            raise ValueError("Unsharded publication requires one writer.")
    ...
    def as_manifest(self) -> dict[str, object]:
        return {
            "schema_id": SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_ID,
            "schema_version": SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_VERSION,
...
def plan_subject_mask_quality_storage(
    dimensions: SubjectMaskQualityDimensions,
    *,
    ...
    rules = {rule.path: rule for rule in SUBJECT_MASK_QUALITY_STORAGE_RULES}
    if tuple(rules) != SUBJECT_MASK_QUALITY_SCHEMA_V1.binding_paths:
        raise ValueError("Storage rules do not match the quality schema.")
    entries: list[SubjectMaskQualityStoragePlanEntry] = []
    for binding in SUBJECT_MASK_QUALITY_SCHEMA_V1.bindings:
        ...
        entries.append(
            SubjectMaskQualityStoragePlanEntry(
                rule=rule,
                plan=plan_storage(intent, profile),
            )
        )
    return SubjectMaskQualityStoragePlanSet(
...
__all__ = [
    "SUBJECT_MASK_QUALITY_STORAGE_RULES",
    "SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_ID",
    "SUBJECT_MASK_QUALITY_STORAGE_SCHEMA_VERSION",
    "SubjectMaskQualityStoragePlanEntry",
    "SubjectMaskQualityStoragePlanSet",
    "SubjectMaskQualityStorageRule",
