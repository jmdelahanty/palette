...
class AcquisitionCropLedgerPublication:
    ...
    blank_row_count: int
    imported_at_utc: str
    source_profile: str = ACQUISITION_CROP_SOURCE_PROFILE_SINGLE
    ...
    def attrs(self) -> dict[str, Any]:
        return {
            ...
            "canonical_ledger_source_profile": self.source_profile,
...
def _compact_utc_timestamp(value: str) -> str:
    try:
        ...
        raise ValueError(
            f"imported_at_utc must be an ISO-8601 timestamp: {value!r}"
        ) from exc
    ...
    *,
    ...
    for name in ("metadata", "keyframes", "summary", "status"):
        ...
        if isinstance(value, str) and value.strip():
            declared[name] = value
    if "status" not in declared and declared.get("summary", "").endswith(
        "_summary.json"
    ):
        declared["status"] = (
            declared["summary"][: -len("_summary.json")] + "_status.json"
        )
...
def _parse_crop_metadata(
    path: Path,
    *,
    width: int,
    height: int,
    expected_first_frame_id: int = 1,
) -> _ParsedLedger:
    ...
    with path.open("r", encoding="utf-8", newline="") as handle:
        ...
        previous_frame_id = int(expected_first_frame_id) - 1
        for row_index, row in enumerate(reader):
            row_number = row_index + 2
            frame_id = _strict_int(
                row.get("recording_frame_id"),
                column="recording_frame_id",
            ...
            crop_rect_valid = (
                _strict_bool(
                    row.get("crop_rect_valid"),
                    column="crop_rect_valid",
                    row_number=row_number,
                )
            ...
            if (
                crop_rect_valid != crop_valid_default
                or detection_rect_valid != crop_valid_default
            ):
                raise ValueError(
            ...
            if crop_rect_valid:
                if not all(math.isfinite(value) for value in crop_xywh):
                    raise ValueError(
                        f"Crop metadata row {row_number} has non-finite crop geometry."
                    )
                ...
                if crop_xywh[0] < 0 or crop_xywh[1] < 0:
                    raise ValueError(
                        f"Crop metadata row {row_number} has a negative crop origin."
                    )
    ...
    arrays = {
        "source_crop_meta_row_indices": np.asarray(
            columns["source_crop_meta_row_indices"], dtype=np.int64
        ),
        "source_recording_frame_ids": np.asarray(
            columns["source_recording_frame_ids"], dtype=np.int64
        ),
        "source_recording_frame_indices": np.asarray(
            columns["source_recording_frame_indices"], dtype=np.int64
        ),
        "source_crop_video_frame_indices": np.asarray(
            columns["source_crop_video_frame_indices"], dtype=np.int64
        ),
        "source_session_crop_video_frame_indices": np.asarray(
            columns["source_session_crop_video_frame_indices"], dtype=np.int64
        ),
        "source_crop_local_frame_ids": np.asarray(
            columns["source_crop_local_frame_ids"], dtype=np.int64
        ),
        "source_camera_frame_ids": np.asarray(
            columns["source_camera_frame_ids"], dtype=np.int64
        ),
        "source_timestamp_ns": np.asarray(
            columns["source_timestamp_ns"], dtype=np.int64
        ),
        "source_timestamp_sys_ns": np.asarray(
            columns["source_timestamp_sys_ns"], dtype=np.int64
        ),
        ...
        "source_crop_xywh": np.asarray(
            columns["source_crop_xywh"], dtype=np.float64
        ).reshape(-1, 4),
        "source_detection_xywh": np.asarray(
            columns["source_detection_xywh"], dtype=np.float64
        ).reshape(-1, 4),
...
def _collection_media_identity(
    path: Path,
    *,
    recording_dir: Path,
    role: str,
    ...
    attrs = source_stat_fingerprint_attrs(
        path,
        attr_prefix="media",
        extra={"role": role, "member_index": int(member_index)},
    )
    try:
        declared_path = str(path.resolve().relative_to(recording_dir.resolve()))
    except ValueError:
        declared_path = str(path.resolve())
    return {
        "path": declared_path,
        "resolved_path": str(path.resolve()),
        "strategy": attrs["media_fingerprint_strategy"],
        "fingerprint": attrs["media_fingerprint"],
        "size_bytes": attrs["media_size_bytes"],
        "mtime_ns": attrs["media_mtime_ns"],
    }
    ...
    for path in metadata_paths:
        with path.open("r", encoding="utf-8", newline="") as handle:
            ...
            for row_number, row in enumerate(reader, start=2):
                has_detection = _strict_bool(
                    row.get("has_detection"),
                    column="has_detection",
                    row_number=row_number,
                )
                blank_frame = _strict_bool(
                    row.get("blank_frame"),
                ...
                if not has_detection or blank_frame:
                    continue
                width = _strict_int(
                    row.get("crop_w"), column="crop_w", row_number=row_number
                )
                height = _strict_int(
                    row.get("crop_h"), column="crop_h", row_number=row_number
                )
                if width <= 0 or height <= 0:
                    raise ValueError(
                        f"Crop metadata row {row_number} has a non-positive crop size."
                    )
                return width, height
    raise ValueError(
        "Rolling crop collection contains no row with valid crop geometry."
    )


def _merge_collection_ledgers(
    parsed_members: list[_ParsedLedger],
    *,
    member_row_counts: list[int],
) -> _ParsedLedger:
    if not parsed_members:
        raise ValueError("Rolling crop collection contains no members.")
    fieldnames = parsed_members[0].fieldnames
    for member in parsed_members[1:]:
        if member.fieldnames != fieldnames:
            raise ValueError(
                "Rolling crop metadata members do not have one exact CSV column contract."
            )

    code_maps: dict[str, dict[str, int]] = {}
    arrays: dict[str, np.ndarray] = {}
    categorical_arrays = {f"{name}_codes" for name in _CATEGORICAL_COLUMNS}
    for name in _CATEGORICAL_COLUMNS:
        vocabulary = sorted(
            {value for member in parsed_members for value in member.code_maps[name]}
        )
        global_map = {value: index for index, value in enumerate(vocabulary)}
        recoded: list[np.ndarray] = []
        for member in parsed_members:
            inverse = {code: value for value, code in member.code_maps[name].items()}
            local_codes = np.asarray(member.arrays[f"{name}_codes"], dtype=np.int16)
            lookup = np.asarray(
                [global_map[inverse[index]] for index in range(len(inverse))],
                dtype=np.int16,
            )
            recoded.append(lookup[local_codes])
        arrays[f"{name}_codes"] = np.concatenate(recoded)
        code_maps[name] = global_map

    for name in _ARRAY_NAMES:
        if name in categorical_arrays:
            continue
        arrays[name] = np.concatenate(
            [np.asarray(member.arrays[name]) for member in parsed_members], axis=0
        )

    arrays["source_clip_indices"] = np.concatenate(
        [
            np.full(row_count, member_index, dtype=np.int32)
            for member_index, row_count in enumerate(member_row_counts)
        ]
    )
    arrays["source_crop_video_member_indices"] = np.asarray(
        arrays["source_clip_indices"], dtype=np.int32
    ).copy()
    arrays["source_full_video_member_indices"] = np.asarray(
        arrays["source_clip_indices"], dtype=np.int32
    ).copy()
    return _ParsedLedger(
        fieldnames=fieldnames,
        arrays=arrays,
        code_maps=code_maps,
        row_count=int(sum(member_row_counts)),
        detected_row_count=int(sum(item.detected_row_count for item in parsed_members)),
        blank_row_count=int(sum(item.blank_row_count for item in parsed_members)),
    )


def _validated_collection_contract(
    recording_dir: Path,
    manifest: Mapping[str, Any],
) -> tuple[dict[str, Any], list[dict[str, Any]], _ParsedLedger]:
    profile = manifest.get("rolling_clip_streams")
    if not isinstance(profile, Mapping):
        raise ValueError("Recording manifest has no rolling_clip_streams contract.")
    if profile.get("schema_id") != "palette.orange_rolling_clip_streams.v1":
        raise ValueError("Unsupported rolling_clip_streams schema.")
    if profile.get("frame_clock") != "recording_frame_id":
        raise ValueError(
            "Rolling crop collection requires recording_frame_id frame clock."
        )

    index_value = profile.get("recording_clip_index") or manifest.get(
        "recording_clip_index"
    )
    index_path = _resolve_relative(
        recording_dir, index_value, label="recording_clip_index"
    )
    if not index_path.is_file():
        raise ValueError(f"Recording clip index does not exist: {index_path}")
    index = _read_json_object(index_path, label="recording clip index")
    if index.get("schema_id") != "palette.orange_external_ipc_recording_clip_index.v1":
        raise ValueError("Unsupported recording clip index schema.")
    if index.get("mode") != "rolling_clips":
        raise ValueError("Recording clip index does not declare rolling_clips mode.")
    camera_id = str(manifest.get("camera_id") or "").strip()
    if not camera_id:
        raise ValueError(
            "Rolling crop collection requires recording_manifest camera_id."
        )
    raw_rows = index.get("rows")
    if not isinstance(raw_rows, list):
        raise ValueError("Recording clip index has no rows array.")
    camera_rows = [
        row
        for row in raw_rows
        if isinstance(row, Mapping)
        and str(row.get("camera_serial") or "").strip() == camera_id
    ]
    camera_rows.sort(key=lambda row: int(row.get("clip_index", -1)))
    if not camera_rows:
    ...
    for expected_member_index, row in enumerate(camera_rows):
        ...
        if clip_index != expected_member_index:
            raise ValueError(
                "Rolling crop collection clip_index values must be contiguous and zero-based."
            )
        first_frame_id = int(row.get("first_recording_frame_id", -1))
        last_frame_id = int(row.get("last_recording_frame_id", -1))
        frame_count = int(row.get("frame_count", -1))
        if (
            first_frame_id != expected_first
            or last_frame_id - first_frame_id + 1 != frame_count
        ):
            raise ValueError(
                f"Clip {clip_index} has a non-contiguous recording-frame interval."
            )
        expected_first = last_frame_id + 1
        clip_manifest_value = row.get("clip_manifest_path")
        clip_manifest_path = _resolve_relative(
            recording_dir, clip_manifest_value, label="clip_manifest_path"
        )
        clip_manifest = _read_json_object(clip_manifest_path, label="clip manifest")
        if int(clip_manifest.get("clip_index", -1)) != clip_index:
            raise ValueError(f"Clip manifest index mismatch for clip {clip_index}.")
        outputs = clip_manifest.get("recording_outputs")
        camera_outputs = (
            outputs.get(camera_id) if isinstance(outputs, Mapping) else None
        )
        if not isinstance(camera_outputs, Mapping):
            raise ValueError(
                f"Clip {clip_index} has no outputs for camera {camera_id}."
            )
        crop = camera_outputs.get("crop")
        ...
        for role, output in (("crop", crop), ("full", full)):
            if int(output.get("first_recording_frame_id", -1)) != first_frame_id:
                raise ValueError(
                    f"Clip {clip_index} {role} first frame differs from index."
                )
            if int(output.get("last_recording_frame_id", -1)) != last_frame_id:
                raise ValueError(
                    f"Clip {clip_index} {role} last frame differs from index."
                )
            if int(output.get("frame_count", -1)) != frame_count:
                raise ValueError(
                    f"Clip {clip_index} {role} frame count differs from index."
                )
        crop_video = _resolve_relative(
            recording_dir, crop.get("video"), label="crop video"
        )
        crop_metadata = _resolve_relative(
            recording_dir, crop.get("metadata"), label="crop metadata"
        )
        full_video = _resolve_relative(
            recording_dir, full.get("video"), label="full video"
        )
        for label, path in (
            ("crop video", crop_video),
            ("crop metadata", crop_metadata),
            ("full video", full_video),
        ):
            if not path.is_file():
                raise ValueError(f"Clip {clip_index} {label} does not exist: {path}")
        unresolved_members.append(
            {
                "member_index": expected_member_index,
                "clip_index": clip_index,
                "clip_id": str(
                    row.get("clip_id") or clip_manifest.get("clip_id") or ""
                ),
                "first_recording_frame_id": first_frame_id,
                "last_recording_frame_id": last_frame_id,
    ...
    for member in unresolved_members:
        parsed = _parse_crop_metadata(
            member["crop_metadata_path"],
            width=crop_width,
            height=crop_height,
            expected_first_frame_id=member["first_recording_frame_id"],
        )
        if parsed.row_count != member["frame_count"]:
            raise ValueError(
                f"Clip {member['clip_index']} crop metadata row count differs from its frame count."
            )
        expected_session_indices = np.arange(
            member["first_recording_frame_id"] - 1,
            member["last_recording_frame_id"],
            dtype=np.int64,
        )
        if not np.array_equal(
            parsed.arrays["source_session_crop_video_frame_indices"],
            expected_session_indices,
        ):
            raise ValueError(
                f"Clip {member['clip_index']} session crop frame indices do not match the recording frame clock."
            )
        ...
        members.append(
            {
                ...
                "first_recording_frame_id": member["first_recording_frame_id"],
                "last_recording_frame_id": member["last_recording_frame_id"],
                "frame_count": member["frame_count"],
                "clip_manifest_path": str(member["clip_manifest_path"]),
                "clip_manifest_sha256": _sha256_file(member["clip_manifest_path"]),
                "crop_metadata_path": str(member["crop_metadata_path"]),
                "crop_metadata_sha256": _sha256_file(member["crop_metadata_path"]),
                "crop_video": _collection_media_identity(
                    member["crop_video_path"],
                    recording_dir=recording_dir,
                    role="crop",
                    member_index=member["member_index"],
                ),
                "full_video": _collection_media_identity(
                    member["full_video_path"],
                    recording_dir=recording_dir,
                    role="full",
                    member_index=member["member_index"],
                ),
    ...
    parsed = _merge_collection_ledgers(
        parsed_members,
        member_row_counts=[member["frame_count"] for member in unresolved_members],
    )
    collection_contract = {
        ...
        "recording_clip_index_sha256": _sha256_file(index_path),
        "recording_clip_index_schema_id": index.get("schema_id"),
    }
    return collection_contract, members, parsed
...
def _validated_contract(
    ...
    if video_streams.get("frame_clock") != "recording_frame_id":
        raise ValueError(
            "Crop ledger import requires recording_frame_id as the frame clock."
        )
    ...
    if not manifest_camera or not crop_camera or manifest_camera != crop_camera:
        raise ValueError(
            "Crop stream camera_id must exactly match recording_manifest camera_id."
        )
    try:
        ...
        raise ValueError(
            "Crop stream width, height, and frame_count must be integers."
        ) from exc
    if width <= 0 or height <= 0 or frame_count < 0:
        raise ValueError(
            "Crop stream width/height must be positive and frame_count non-negative."
        )
    metadata_path = _resolve_relative(
        recording_dir, crop.get("metadata"), label="metadata"
    )
...
def _validate_current_run(
    stream_group: Any, publication: AcquisitionCropLedgerPublication
) -> None:
    ...
    for name in _ARRAY_NAMES:
        if name not in run or int(run[name].shape[0]) != publication.row_count:
            raise ValueError(
                f"Current crop ledger array {name!r} is missing or incomplete."
            )
    if publication.source_profile == ACQUISITION_CROP_SOURCE_PROFILE_COLLECTION:
        for name in _COLLECTION_ARRAY_NAMES:
            if name not in run or int(run[name].shape[0]) != publication.row_count:
                raise ValueError(
                    f"Current collection crop ledger array {name!r} is missing or incomplete."
                )
        members = run.attrs.get("source_media_members")
        if not isinstance(members, list) or not members:
            raise ValueError(
                "Current collection crop ledger has no source member table."
            )
    ...
    *,
    ...
    if current_digest or current_run:
        ...
        existing = AcquisitionCropLedgerPublication(
            ...
            source_metadata_sha256=metadata_sha256,
            source_video_fingerprint=str(
                video_fingerprint_attrs["source_video_fingerprint"]
            ),
            row_count=parsed.row_count,
            detected_row_count=parsed.detected_row_count,
            blank_row_count=parsed.blank_row_count,
            imported_at_utc=str(
                stream_group.attrs.get("canonical_ledger_imported_at_utc") or now
            ),
            source_profile=ACQUISITION_CROP_SOURCE_PROFILE_SINGLE,
            idempotent=True,
        )
...
def publish_acquisition_crop_stream_collection_ledger(
    stream_group: Any,
    recording_dir: Path,
    manifest: Mapping[str, Any],
    *,
    ...
    """Publish one immutable recording-level ledger for an ordered clip collection."""
    ...
    contract, members, parsed = _validated_collection_contract(recording_path, manifest)
    metadata_digest = hashlib.sha256(
        _canonical_json([member["crop_metadata_sha256"] for member in members]).encode(
            "utf-8"
        )
    ).hexdigest()
    media_digest = hashlib.sha256(
        _canonical_json(
            [
                {
                    "member_index": member["member_index"],
                    "crop": member["crop_video"],
                    "full": member["full_video"],
                }
    ...
    record = {
        "schema_id": ACQUISITION_CROP_COLLECTION_LEDGER_SCHEMA_ID,
        "schema_version": ACQUISITION_CROP_COLLECTION_LEDGER_SCHEMA_VERSION,
        "source_profile": ACQUISITION_CROP_SOURCE_PROFILE_COLLECTION,
        "source_metadata_sha256": metadata_digest,
        "source_video_fingerprint": media_digest,
        "source_stream_contract": contract,
        "source_media_members": members,
        "source_columns": list(parsed.fieldnames),
        "row_count": parsed.row_count,
    }
    record_sha256 = hashlib.sha256(_canonical_json(record).encode("utf-8")).hexdigest()
    ...
    compact_time = _compact_utc_timestamp(now)
    run_name = f"crop_collection_ledger_{compact_time}_{record_sha256[:12]}"
    runs = stream_group.require_group(ACQUISITION_CROP_LEDGER_RUNS_GROUP)
    if run_name in runs:
        raise ValueError(f"Crop collection ledger run collision: {run_name}")
    run = runs.create_group(run_name)
    run.attrs.update(
        {
            "schema_id": ACQUISITION_CROP_COLLECTION_LEDGER_SCHEMA_ID,
            ...
            "source_stream_contract": contract,
            "source_media_members": members,
            "source_media_member_count": len(members),
            "row_count": parsed.row_count,
            "created_at_utc": now,
        }
    )
    ...
    try:
        for name, values in parsed.arrays.items():
            create_geometry_preload_array(
                ...
                attrs={
                    "row_axis_semantics": (
                        "ordered_recording_frame_across_acquisition_clip_members"
                    )
                },
            )
        run.attrs.update(
...
def validate_current_acquisition_crop_stream_ledger(
    ...
    *,
    ...
    if expected_record_sha256 is not None and record_sha256 != expected_record_sha256:
        raise ValueError(
            "Canonical crop ledger digest differs from the expected publication."
        )
    publication = AcquisitionCropLedgerPublication(
        status="complete",
        ...
        source_profile=str(
            attrs.get("canonical_ledger_source_profile")
            or ACQUISITION_CROP_SOURCE_PROFILE_SINGLE
        ),
    )
...
__all__ = [
    ...
    "ACQUISITION_CROP_SOURCE_PROFILE_SINGLE",
    "AcquisitionCropLedgerPublication",
