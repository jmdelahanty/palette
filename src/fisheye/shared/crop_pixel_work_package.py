...
class CropPixelWorkPackageArray:
    ...
    def __init__(self, path: Path, *, shape: tuple[int, int, int]) -> None:
        ...
        if not self.path.is_file():
            raise FileNotFoundError(f"Crop pixel work-package payload is missing: {self.path}")
...
def _normalize_target_rows(values: Sequence[int] | np.ndarray, *, total_rows: int) -> np.ndarray:
    rows = np.asarray(values, dtype=np.int64).reshape(-1)
    if rows.size == 0:
        raise CropPixelWorkPackageError("Crop pixel work package requires at least one row.")
...
def build_crop_pixel_work_package_from_source(
    ...
    *,
    ...
    if np.unique(keys).shape[0] != keys.shape[0]:
        raise CropPixelWorkPackageError("Selected crop instance_key values are not unique.")
    ...
    if signatures.shape != (rows.shape[0], ROW_SOURCE_SIGNATURE_WIDTH_BYTES):
        raise CropPixelWorkPackageError("Selected crop source signatures have invalid shape.")
    ...
    try:
        with np.load(path, allow_pickle=False) as payload:
            ...
            if schema_id != CROP_PIXEL_WORK_PACKAGE_ROWS_SCHEMA_ID:
                raise CropPixelWorkPackageError("Unsupported work-package row schema id.")
            if schema_version != CROP_PIXEL_WORK_PACKAGE_ROWS_SCHEMA_VERSION:
                raise CropPixelWorkPackageError("Unsupported work-package row schema version.")
            return {
                "crop_row_indices": np.asarray(payload["crop_row_indices"], dtype=np.int64),
...
def _validate_live_binding(
    ...
    *,
    ...
    if "instance_key" not in crop or ROW_SOURCE_SIGNATURE_ARRAY not in crop:
        raise CropPixelWorkPackageError("Bound crop run lacks modern identity/signatures.")
    ...
    if not np.array_equal(keys, package.instance_keys):
        raise CropPixelWorkPackageError("Work-package keys no longer match the crop run.")
    ...
    if "frame_indices" not in crop or "roi_coordinates_full" not in crop:
        raise CropPixelWorkPackageError("Bound crop run lacks required geometry arrays.")
    if not np.array_equal(
        np.asarray(crop["frame_indices"][rows], dtype=np.int64),
        package.frame_indices,
    ):
        raise CropPixelWorkPackageError("Work-package frames no longer match the crop run.")
    if not np.array_equal(
...
def open_crop_pixel_work_package(
    ...
    *,
    ...
    if manifest.get("schema_id") != CROP_PIXEL_WORK_PACKAGE_SCHEMA_ID:
        raise CropPixelWorkPackageError("Unsupported crop pixel work-package schema id.")
    if int(manifest.get("schema_version", -1)) != CROP_PIXEL_WORK_PACKAGE_SCHEMA_VERSION:
        raise CropPixelWorkPackageError("Unsupported crop pixel work-package schema version.")
    ...
    if expected_archive_path is not None and Path(
        str(source.get("archive_path") or "")
    ).expanduser().resolve() != Path(expected_archive_path).expanduser().resolve():
    ...
    if expected_crop_run is not None and str(source.get("crop_run_name") or "") != str(
        ...
        raise CropPixelWorkPackageError("Work package is bound to a different crop run.")
    ...
    if min(shape) <= 0 or str(array.get("dtype")) != "uint8" or array.get("order") != "C":
    ...
    rows_path = _resolve_relative(manifest_file, rows_meta.get("path"), field="rows.path")
    if not rows_path.is_file():
        raise FileNotFoundError(f"Crop pixel work-package rows are missing: {rows_path}")
    ...
    _normalize_target_rows(
        ...
        total_rows=int(_require_mapping(manifest, "selection")["source_crop_total_rows"]),
    )
    if np.unique(rows["instance_key"]).shape[0] != row_count:
        raise CropPixelWorkPackageError("Work-package instance_key values are not unique.")
    pixels = CropPixelWorkPackageArray(payload_path, shape=shape)
    try:
        if verify_payload and _sha256_file(payload_path) != str(array.get("sha256") or ""):
            raise CropPixelWorkPackageError("Work-package pixel payload digest mismatch.")
        package = CropPixelWorkPackage(
        ...
        if package.package_id != expected_id:
            raise CropPixelWorkPackageError("Work-package logical package_id is invalid.")
...
def cleanup_unreferenced_crop_pixel_work_package_generations(
    ...
    *,
    ...
    try:
        ...
        referenced = {
            _resolve_relative(manifest_file, array.get("bin_path"), field="array.bin_path"),
