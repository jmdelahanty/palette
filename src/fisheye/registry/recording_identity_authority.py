...
REQUIRED_SCHEMA_FINGERPRINT = (
    "492fc77ad12410b247a5ce53a41533251b01745368cedda891d21be38fa28892"
)
...
def _schema_ready(conn: sqlite3.Connection) -> int:
    ...
    if schema_version < 72:
        raise RecordingIdentityAuthorityError(
            "recording identity authority requires registry schema version 72 or later"
        )
    migration = conn.execute(
        "SELECT name FROM schema_version WHERE version = 72;"
    ).fetchone()
    ...
    if canonical_json_sha256(schema_payload) != REQUIRED_SCHEMA_FINGERPRINT:
        raise RecordingIdentityAuthorityError(
            "recording identity authority schema fingerprint does not match migration 72"
        )
