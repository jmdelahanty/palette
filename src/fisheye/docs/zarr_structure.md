...
Attributes: `source_crop_run`, `source_keypoint_run`, `source_keypoint_group`, `method`, model info,
thresholds, smoothing parameters, aggregate metrics, `reason_counts`.
...
- Scalar QA arrays such as `area_refined`, `area_delta_vs_source`,
  ...
  `refined`, `copied`, `filtered_*`, `retuned`, `manual_correction`).
...
(`eye_mask_review --retune/--manual`). `retune_params` maps `retune_id` values
to the parameter sets applied during batch retuning.
