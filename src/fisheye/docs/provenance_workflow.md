...
| `crop_runs/<run>` | `roi_images`, `source_refined_row_ids` *(when sourced from canonical refined detect)* | `detection_source_path`, `detect_review_status_ref`, `detect_review_status` (snapshot), `detection_selection_policy`, `crop_signature`, `roi_image_representation`, `roi_pixel_contract`, `crop_review_status` |
