...
def build_plan(
    *,
    ...
    if scope in {WORKFLOW_SCOPE_FULL, WORKFLOW_SCOPE_KEYPOINTS}:
    ...
    for target_index, target in enumerate(targets):
        ...
        target_payload: dict[str, Any] = {
            **target.to_json(),
            ...
            "keypoint_run": keypoint_run if scope == WORKFLOW_SCOPE_FULL else None,
            "refined_keypoint_run": (
                refined_keypoint_run if scope == WORKFLOW_SCOPE_FULL else None
            ),
            "subject_mask_run": (
                subject_mask_run if scope == WORKFLOW_SCOPE_FULL else None
            ),
            "refined_subject_mask_run": (
                refined_subject_mask_run if scope == WORKFLOW_SCOPE_FULL else None
            ),
            ...
            "subject_mask_quality_run": (
                subject_mask_quality_run if scope == WORKFLOW_SCOPE_FULL else None
            ),
            "subject_mask_cache_run": (
                subject_mask_cache_run if scope == WORKFLOW_SCOPE_FULL else None
            ),
            "subject_mask_bundle_id": (
                subject_mask_bundle_id if scope == WORKFLOW_SCOPE_FULL else None
            ),
            "clips": clips,
            "registered_dish_geometry": {
...
def _parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "keypoint publication chain, or stop after the canonical "
            "recording-level refined-detection snapshot."
        ),
    )
    ...
    parser.add_argument(
        "--pose-set-id", help="Required for --workflow-scope full or keypoints."
    )
    parser.add_argument(
        "--pose-run-id", help="Required for --workflow-scope full or keypoints."
    )
    parser.add_argument(
