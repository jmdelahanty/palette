...
def _bout_kinematics_command(plan: ArchivePlan, args: argparse.Namespace) -> list[str]:
    ...
    if plan.include_eye_gaze:
        ...
        cmd.append("--no-include-eye-gaze")
    if args.overwrite:
...
def _validate_plan_outputs(plan: ArchivePlan) -> tuple[str, str]:
    ...
    any_stage_planned = any(
        (
            plan.run_arena_assignment,
            plan.run_track_kinematics,
            plan.run_track_visualization,
            plan.run_eye_angles,
            plan.run_swim_bouts,
            plan.run_bout_kinematics,
        )
    )
    # When used as a standalone validator in tests or diagnostics, preserve the
    # historical behavior of checking the full movement/bout target surface. In
    # batch apply mode, validate only the stages this plan actually generated
    # plus their required dependencies; otherwise an eye-angle-only backfill can
    # fail because a pre-existing compatibility bout-kinematics run lacks
    # eye-gaze metrics that were not regenerated by this plan.
    validate_full_surface = not any_stage_planned
    checks: list[str] = []
    needs_track = validate_full_surface or any(
        (
            plan.run_track_kinematics,
            plan.run_track_visualization,
            plan.run_swim_bouts,
    ...
    if needs_track:
        checks.append(f"analysis/track_kinematics_runs/offline/{plan.track_run}")
    if plan.run_eye_angles or (plan.run_bout_kinematics and plan.include_eye_gaze):
        checks.append(f"analysis/eye_angle_runs/{plan.eye_angle_run}")
    missing = [rel for rel in checks if not _exists(zarr_path, rel)]
    if (validate_full_surface or plan.run_swim_bouts or plan.run_bout_kinematics) and not _swim_bout_logical_bouts_exist(
        zarr_path, plan.swim_bout_run
    ):
        missing.append(f"analysis/swim_bout_runs/{plan.swim_bout_run} logical bouts")
    if (validate_full_surface or plan.run_bout_kinematics) and not _bout_kinematics_logical_level_exists(
        zarr_path, plan.bout_kinematics_run, "movement"
    ):
        missing.append(f"analysis/bout_kinematics_runs/{plan.bout_kinematics_run} logical movement metrics")
    if (plan.run_bout_kinematics and plan.include_eye_gaze) and not _bout_kinematics_logical_level_exists(
