...
def _check_zarr(zarr_path: Path, tuning_keys: List[str]) -> Dict[str, object]:
    ...
    if refined_keypoints_parent is not None:
        latest_refined_keypoints = refined_keypoints_parent.attrs.get("latest")
        if latest_refined_keypoints and latest_refined_keypoints in refined_keypoints_parent:
            ...
            keypoint_review_status = _coerce_mapping(
                refined_keypoints_parent[latest_refined_keypoints].attrs.get("keypoint_review_status")
            )
        else:
...
def _status_rich(value: bool) -> str:
...
def _refined_status_rich(coverage: Optional[float], method: Optional[str]) -> str:
    if coverage is None:
        return "[red]MISS[/red]"
    if coverage >= 99.999:
        percent = "[chartreuse1]100%[/chartreuse1]"
    else:
        percent = f"[yellow]{coverage:.1f}%[/yellow]"
    if method:
...
def main(argv: Optional[List[str]] = None) -> int:
    ...
    if use_rich:
        ...
        table.add_column("Refine Keypoints")
        ...
        for plan in plans:
            import_ok = plan.raw_present and (plan.full_present or plan.ds_present)
            ...
            row = [
                ...
                _status_rich(plan.background_full_present),
                _status_rich(plan.background_ds_present),
                ...
                _status_rich(plan.refined_keypoints_present),
            ...
            for key in tuning_keys:
                row.append(_tuning_status_rich(plan.tuning_status.get(key, "miss")))
            table.add_row(*row)
        console.print(table)
    else:
        for plan in plans:
            ...
            print(f"  background_full: {_status_text(plan.background_full_present)}")
            print(f"  background_ds: {_status_text(plan.background_ds_present)}")
            ...
            print(f"  refined_keypoints: {_status_text(plan.refined_keypoints_present)}")
            ...
            for key in tuning_keys:
                print(f"    {key}: {_tuning_status_text(plan.tuning_status.get(key, 'miss'))}")
