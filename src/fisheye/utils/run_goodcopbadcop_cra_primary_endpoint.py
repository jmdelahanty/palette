"""Run GoodCopBadCop CRA primary endpoint components across recordings."""
        ...
        results.append(
            {
                ...
                "component_name": component_name,
                "cra_primary_endpoint_path": applied_path,
                "status": status,
                "error": error,
                "summary": summary,
            }
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.json:
        ...
        for row in results:
            status = row["status"]
            path_or_run = row["cra_primary_endpoint_path"] or row["component_name"]
        ...
        if not args.apply:
            print("dry_run: pass --apply to write CRA primary endpoint components")
    return 0
