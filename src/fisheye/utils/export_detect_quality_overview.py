#!/usr/bin/env python3
"""Export saved detect-quality overview PNG artifacts from refined detect runs."""
...
REFINEMENT_ARTIFACT_NAME = "refinement_pipeline_overview_png"
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Export detect-quality overview PNG artifacts stored in refined detect runs."
    )
    parser.add_argument(
        "paths",
        nargs="*",
    ...
    parser.add_argument(
        "--refined-run",
        help="Specific refined run to export from (default: refined parent latest).",
    )
    parser.add_argument(
        "--artifact",
        choices=[ARTIFACT_NAME, REFINEMENT_ARTIFACT_NAME],
    ...
    parser.add_argument(
        "--output-dir",
        type=Path,
        default=Path("/tmp/detect_quality_overview_exports"),
    ...
    for row in rows:
        if row.status != "ready":
            ...
            continue
        assert row.parent_name is not None
        ...
        try:
            root = zarr.open_group(row.zarr_path, mode="r")
            vis_array = root[row.parent_name][row.refined_run]["visualizations"][row.artifact_name]
            png_bytes = np.asarray(vis_array[:], dtype=np.uint8).tobytes()
    ...
    if args.list:
        print(
            "Detect-quality artifact export: "
