...
def _run_exists(root: zarr.Group, run_name: Optional[str]) -> bool:
    ...
    if analysis is None:
        return False
    parent = _get_group(analysis, "eye_mask_profile_runs")
    if parent is None:
        return False
...
def _format_row(row: BackfillRow) -> str:
    source = row.source_eye_mask_path or "-"
...
def _default_run_name_from_created(created_at_utc: object) -> Optional[str]:
    ...
    if not date_part or not time_part:
        return None
    return f"eye_mask_profile_{date_part}_{time_part}".replace(":", "-")
    ...
    *,
    ...
    if _run_exists(root, run_name) and not overwrite_existing:
        return BackfillRow(
            ...
            reason=f"analysis/eye_mask_profile_runs/{run_name} already exists",
            run_name=run_name,
        )
    ...
    try:
        ...
        if apply:
            ...
            return BackfillRow(
                ...
                source_eye_mask_path=result.source_eye_mask_path,
            )
        ...
        source_map = summary.get("source", {})
        return BackfillRow(
            zarr_path=zarr_path,
            status="would_write",
            run_name=computed_run_name,
            source_eye_mask_path=str(source_map.get("eye_mask_path") or ""),
        )
    ...
    except (EyeMaskProfileError, FileExistsError, ValueError) as exc:
        return BackfillRow(
            zarr_path=zarr_path,
            status="error",
            reason=str(exc),
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description=(
            "Backfill canonical eye-mask profile summaries into "
            "analysis/eye_mask_profile_runs/<run>/attrs['profile_summary']."
        )
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help="Optional explicit source path (e.g. eye_masks_runs/<run> or refined_eye_masks_runs/<run>).",
    )
    parser.add_argument("--eye-mask-run", type=str, help="Optional eye-mask run override.")
    parser.add_argument("--refined-run", type=str, help="Optional refined eye-mask run override.")
    ...
    try:
        for zarr_path in _iter_zarr(roots, recursive=bool(args.recursive)):
            ...
            row = _process_zarr_path(
                ...
                eye_mask_run=args.eye_mask_run,
