#!/usr/bin/env python3
"""Sync latest detection profile summaries from Zarr into registry rows."""
...
def _latest_profile_summary(root: zarr.Group) -> tuple[Optional[str], Optional[dict[str, Any]], Optional[str]]:
    ...
    if runs_parent is None:
        return None, None, "analysis/detection_profile_runs missing"
    run_name = _select_latest_profile_run(runs_parent)
    if run_name is None:
        return None, None, "analysis/detection_profile_runs has no runs"
...
def _build_profile_payload(
    *,
    ...
    geometry = summary.get("geometry_norm")
    ...
    counts_map = dict(counts) if isinstance(counts, Mapping) else {}
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Sync latest detection profile summaries from Zarr into detection_data_profile registry rows."
    )
    ...
    try:
        ...
        print(
            "Detection profile registry sync: "
            f"mode={mode} "
