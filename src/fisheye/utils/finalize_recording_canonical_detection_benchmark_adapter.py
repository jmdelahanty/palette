...
outputs and an earlier logically equivalent recording fixture, rebuilds the
complete recording table through the current canonical binder, writes the
approved access-aware profile on node-local scratch, and atomically places the
...
def _validate_anchor(
    *,
    ...
    if dimensions != bound.dimensions:
    ...
    for path in CANONICAL_DETECTION_SCHEMA_V1.binding_paths:
        expected = np.asarray(bound.arrays[path])
...
def finalize_recording_canonical_detection_benchmark_adapter(
    *,
    ...
    if not archive_path.is_dir() or not anchor_group.is_dir():
        raise FileNotFoundError("Analysis Zarr or canonical anchor is missing.")
    ...
    provenance: dict[str, object] = {
        ...
        "conversion": (
            "legacy_clip_detection_groups_to_native_canonical_v2_benchmark"
        ),
