"""Run GoodCopBadCop CRA near-field avoidance components across recordings."""
...
def run_for_targets(
    ...
    *,
    ...
    cra_primary_endpoint_component: str,
    component_name: str,
    r_zone_mm: float,
    r_in_mm: float,
    r_out_mm: float,
    ...
    for target in targets:
        ...
        try:
            ...
            source_cra_component = result.source_cra_primary_endpoint_component
        ...
        results.append(
            {
                ...
                "cra_primary_endpoint_component": source_cra_component or cra_primary_endpoint_component,
                "component_name": component_name,
                "cra_near_field_path": applied_path,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    if args.json:
        ...
        for row in results:
            status = row["status"]
            path_or_component = row["cra_near_field_path"] or row["component_name"]
            print(
        ...
        if not args.apply:
            print("dry_run: pass --apply to write CRA near-field components")
    return 0
