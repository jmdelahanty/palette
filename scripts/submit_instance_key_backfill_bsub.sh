...
OUTPUT_ROOT="${PALETTE_INSTANCE_KEY_BACKFILL_ROOT:-/groups/johnson/johnsonlab/jeremy/recordings/logs/instance_key_backfill_bsub}"
RUN_ID=""
QUEUE=""
MEM_GB=8
WALLTIME="2:00"
...
usage() {
  cat <<'USAGE'
Usage: submit_instance_key_backfill_bsub.sh --run-id ID --zarr-path PATH... [options]

Render or submit one sequential CPU job that dry-runs or applies the fail-closed
instance_key lineage repair. The login host only submits bsub; all Zarr reads
and writes occur inside the LSF allocation.
...
Options:
  --apply                     Apply after deterministic planning (default: dry-run)
  --palette-repo PATH         Cluster-visible checkout containing the migration
  --source-repo PATH          Local checkout supplying the bundled migration module
  --output-root PATH          Logs, path manifest, and JSON report root
...
SOURCE_MODULE="$SOURCE_REPO/src/fisheye/utils/backfill_legacy_instance_keys.py"
[[ -f "$SOURCE_MODULE" ]] || fail "local migration module not found: $SOURCE_MODULE"
SOURCE_FILES=(
  "$PALETTE_REPO/src/fisheye/shared/instance_keys.py"
  "$PALETTE_REPO/src/fisheye/shared/refined_detect_curation.py"
  "$PALETTE_REPO/src/fisheye/shared/rowset_fingerprint.py"
)
for source_file in "${SOURCE_FILES[@]}"; do
  [[ -f "$source_file" ]] || fail "migration source not found: $source_file"
done
...
JOB_SCRIPT="$RUN_DIR/run_instance_key_backfill.sh"
STATUS_FILE="$RUN_DIR/status.txt"
SUBMISSION_FILE="$RUN_DIR/submission.txt"
BUNDLED_MODULE="$RUN_DIR/backfill_legacy_instance_keys.py"
...
ACTUAL_SOURCE_SHA256="\$(sha256sum \
  "\${BUNDLED_MODULE}" \
  src/fisheye/shared/instance_keys.py \
  src/fisheye/shared/refined_detect_curation.py \
  src/fisheye/shared/rowset_fingerprint.py | awk '{print \$1}' | sha256sum | awk '{print \$1}')"
